...
a very simplified version of Concurrent ML \cite{reppy2007concurrent}. The formal analysis
...
\begin{lstlisting}[language=logic, mathescape]
  ...
  * cons: n$_r$ t graph staticEnv graph env stack' n env .
      {(NRslt n$_r$, MRtn, termId t)} $\subseteq$ graph,
      staticFlowsAccept staticEnv graph t,
      staticFlowsAcceptEnv staticEnv graph env,
...
an additional loop function \lstinline{lp} is added to the
example server implementation. The loop basically just wastes steps,
but it is used to demontrate how
...
The static flows acceptance for higher precision is similar to that of the lower precision analysis.
However, it must additionally consider flows with the sending mode. To accomodate the sending flow
from sender to receiver, the initial program must be carried each inference rule.
...
\begin{lstlisting}[language=logic, mathescape]
  ...
  * sync: n t' graph staticEnv n$_{se}$ .
      ...
      $\rightarrow$ (NBnd n, MSend n$_{se}$, NBnd n$_y$) $\in$ graph),
      staticFlowsAcceptTm staticEnv graph t$_0$ t'
...
If a function is called that does nothing with the channel of interest, then the entry and exit functions
for the sites in that function will be empty; however, the path from before it's called
until after it returns should be considered live all the way through.
For instance, no channel is considered to be live in the body of the loop at \lstinline[language=normal_lang]{bind lp}.
However, a channel may be live before the loop is called and after the loop returns. 

A few additional definitions are needed to describe if a static path is live.
The static path balance checks if a path has balanced call and return flows with
of zero or more sequencuential steps between them.
The static path existence is the same as that for the lower precision analysis.
The static detour describes paths that have live sites before a call flow
and after a return flow, but not in between.


\begin{lstlisting}[language=logic, mathescape]
  predicate staticPathBalanced: static_path -> bool where
    empty:
      staticPathBalanced []  
  * call_return: path id id' .
      staticPathBalanced path
    $\vdash$ staticPathBalanced ([(id, MCll)] @ path @ [(id', MRtn)])  
  * snoc: path id .
      staticPathBalanced path
    $\vdash$ staticPathBalanced (path @ [(id, MSeq)])

  predicate staticPathExists: graph -> (tm_id -> bool) -> static_path -> bool where
    empty: graph isEnd .
    $\vdash$ staticPathExists graph isEnd []
  * edge: graph middle path isEnd end mode .
      staticPathExists graph ($\lambda$ l . l = middle) path,
      isEnd end,
      (middle, mode, end) $\in$ graph 
    $\vdash$ staticPathExists graph isEnd (path @ [(middle, mode)])

  predicate staticDetour:
    graph -> tm_id_map -> tm_id_map -> tm_id
  -> (tm_id -> bool) -> static_path -> bool
  where
    short: isEnd end exit id entr id' graph .
      isEnd end,
      $\neg$ Set.is_empty (exit id),
      Set.is_empty (entr id'),
      Set.is_empty (exit id'),
      $\neg$ Set.is_empty (entr end),
    $\vdash$ staticDetour graph entr exit isEnd [(id, MCll), (id', MRtn)]
  * long: isEnd end exit id entr middle id' end graph mode .
      isEnd end,
      $\neg$ Set.is_empty (exit id),
      Set.is_empty (entr middle),
      Set.is_empty (exit id'),
      $\neg$ Set.is_empty (entr end),
      staticPathExists graph isEnd
        ([(l, MCll), (middle, mode)] @ path @ [(id', MRtn)]),
      staticPathBalanced ((middle, mode) # path),
    $\vdash$  staticDetour graph entr exit isEnd
      ([(id, MCll), (middle, mode)] @ path @ [(id', MRtn)])
...
\begin{lstlisting}[language=logic, mathescape]
  ...
  * detour: graph entr exit start middle path isEnd end detour .
      staticPathLive graph entr exit start ($\lambda$ l . l = middle) path,
      isEnd end,
      staticDetour graph entr exit isEnd ((middle, MSeq) # detour)
    $\vdash$ staticPathLive graph entr exit start isEnd (path @ [(middle, MSeq)] @ detour)
...
\begin{lstlisting}[language=logic, mathescape]
  ...
  * sendLeft: path n path1 path2 .
    $\vdash$ staticInclusive
      (path @ [(NBnd n, MSend n$_e$)] @ path1)
      (path @ [(NBnd n, MSeq)] @ path2)
  * sendRight: path n path1 path2 .
    $\vdash$ staticInclusive
      (path @ [(NBnd n, MSeq)] @ path1)
      (path @ [(NBnd n, MSend n$_e$)] @ path2)
...
\begin{lstlisting}[language=logic, mathescape]
  ...
  * send: t$_0$ pool comm path$_r$ n$_r$ path$_{sfx}$ stt path$_s$ n$_s$ n$_{se}$ t$_{sy}$ env$_{sy}$ stack$_{sy}$
    ...
    $\vdash$ pathsCorrespondModChan t$_0$ chan
      ...
      (staticPath$_{pfx}$ @ [(NBnd n$_s$, MSend n$_{se}$), (NBnd n$_r$, MSeq)] @ staticPath$_{sfx}$)

  * snoc: t$_0$ chan path staticPath site step .
...
The proposition \lstinline[mathescape]{isSendPath pool' (Ch path$_c$ n$_c$) path'} is derived
by unfolding the definition of \lstinline{oneShot}.
It is generalized by \lstinline[mathescape]{pool' path' = Some (Stt t' env' stack'),}\\
\lstinline[mathescape]{dynamicBuiltOnChanState (Stt t' env' stack') (Ch path$_c$ n$_c$)} in the soundess of static path liveness.
...
\begin{lstlisting}[language=logic, mathescape]
  ...
  * stack: stack chan t env .
  ...
  lemma staticTraceableSound: t$_0$ pool comm path t env stack staticEnv staticComm
