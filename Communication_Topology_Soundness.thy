...
lemma exp_not_reachable_sound_for_single: "
  \<lbrakk>
    \<E> \<rightarrow> \<E>'
  \<rbrakk>\<Longrightarrow>
...
"
 apply (erule concur_step.cases, auto)
  using exp_reachable.Refl prefix_order.order_refl same_prefix_nil apply blast
sorry

lemma exp_not_reachable_sound_for_multi: "
  \<lbrakk>
...
lemma exp_not_reachable_sound'': "
  ...
  \<exists>\<pi> e. (\<exists>\<rho> \<kappa>. \<E> \<pi> = Some (\<langle>e;\<rho>;\<kappa>\<rangle>)) \<and> prefix \<pi> \<pi>' \<and> (\<V>, e) \<downharpoonright> e'
"
 apply (drule exp_not_reachable_sound_for_multi, auto)
sorry
...
lemma nonempty_pool_star_left: "
  ...
  (\<forall> \<E>'' . \<E>\<^sub>m \<rightarrow> \<E>'' \<longrightarrow>
    (\<exists> \<pi> e \<rho> \<kappa> \<pi>\<^sub>m e\<^sub>m \<rho>\<^sub>m \<kappa>\<^sub>m. 
      ...
      (\<V>, e) \<downharpoonright> e\<^sub>m \<and>
      leaf \<E>\<^sub>m \<pi>\<^sub>m \<and>
      \<E>'' \<pi>'' = Some (\<langle>e'';\<rho>'';\<kappa>''\<rangle>) \<and>
      prefix \<pi>\<^sub>m \<pi>'' \<and>
      (\<V>, e) \<downharpoonright> e''     
      *)
    )
  )
...
lemma exp_not_reachable_sound': "
  ...
  \<rbrakk> \<Longrightarrow>
  (\<V>, \<C>) \<Turnstile>\<^sub>\<E> \<E> \<longrightarrow>
  (\<forall> \<pi>' e' \<rho>' \<kappa>' \<E>' . \<E> \<rightarrow> \<E>' \<longrightarrow> \<E>' \<pi>' = Some (\<langle>e';\<rho>';\<kappa>'\<rangle>) \<longrightarrow>
    (\<exists> \<pi> e \<rho> \<kappa> \<pi>\<^sub>m e\<^sub>m \<rho>\<^sub>m \<kappa>\<^sub>m.
      \<E> \<pi> = Some (\<langle>e;\<rho>;\<kappa>\<rangle>) \<and>
      prefix \<pi> \<pi>' (* \<and>
      \<E>\<^sub>m \<pi>\<^sub>m = Some (\<langle>e\<^sub>m;\<rho>\<^sub>m;\<kappa>\<^sub>m\<rangle>) \<and>
