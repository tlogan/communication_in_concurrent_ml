...
proof -
  ...
  have "{ | \<omega>\<^sub>m | } \<subseteq> \<V> x\<^sub>r"  using may_be_static_eval_pool_to_precise by blast
...
proof -
  ...
  have  "|VUnit| \<in> \<V> x\<^sub>s" using may_be_static_eval_pool_to_precise by blast
...
proof -
  ...
  have "(\<V>, \<C>) \<Turnstile>\<^sub>\<E> \<E>(\<pi> ;; (LNext x) \<mapsto> \<langle>e'';\<rho>'';\<kappa>\<rangle>)" by (blast intro: may_be_static_eval_preserved_under_concur_step)
...
"
  ...
  apply (drule may_be_static_eval_pool_to_precise, auto)
...
"
  ...
  apply (drule may_be_static_eval_preserved_under_concur_step, auto)
...
"
 ...
 apply (erule star_left.induct; auto)
  ...
  apply (drule may_be_static_eval_preserved_under_concur_step_star, blast)
...
proof -
  ...
  have "(\<V>, \<C>) \<Turnstile>\<^sub>\<E> [[] \<mapsto> \<langle>e\<^sub>0; empty; []\<rangle>]" by (simp add: may_be_static_eval_to_pool)
