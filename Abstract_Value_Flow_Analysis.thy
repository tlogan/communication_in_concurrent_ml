...
inductive flow_over_state :: "abstract_env \<times> abstract_env \<Rightarrow> state \<Rightarrow> bool" (infix "\<TTurnstile>" 55) where
  Result: "
    \<lbrakk>
      (\<V>, \<C>) \<parallel>\<approx> \<rho>;
      (\<V>, \<C>) \<Turnstile> \<V> x \<Rrightarrow> \<kappa>
    \<rbrakk> \<Longrightarrow>
    (\<V>, \<C>) \<TTurnstile> <<RESULT x, \<rho>, \<kappa>>>
  " |
  Let_Unit: "
    \<lbrakk>
      (\<V>, \<C>) \<Turnstile> (LET x = \<lparr>\<rparr> in e); 
      ...
      (\<V>, \<C>) \<Turnstile> \<V> (\<lfloor>e\<rfloor>) \<Rrightarrow> \<kappa>
    \<rbrakk> \<Longrightarrow>
    (\<V>, \<C>) \<TTurnstile> <<LET x = \<lparr>\<rparr> in e, \<rho>, \<kappa>>>
  " |
  Let_Chan: "
    \<lbrakk>
      (\<V>, \<C>) \<Turnstile> (LET x = CHAN \<lparr>\<rparr> in e); 
      ...
      (\<V>, \<C>) \<Turnstile> \<V> (\<lfloor>e\<rfloor>) \<Rrightarrow> \<kappa>
    \<rbrakk> \<Longrightarrow>
    (\<V>, \<C>) \<TTurnstile> <<LET x = CHAN \<lparr>\<rparr> in e, \<rho>, \<kappa>>>
  " |
  Abs: "
    \<lbrakk>
      (\<V>, \<C>) \<Turnstile> (LET x = FN f x\<^sub>l . e\<^sub>l in e); 
      (\<V>, \<C>) \<parallel>\<approx> \<rho>; 
      (\<V>, \<C>) \<Turnstile> \<V> (\<lfloor>e\<rfloor>) \<Rrightarrow> \<kappa>
    \<rbrakk> \<Longrightarrow>
    (\<V>, \<C>) \<TTurnstile> <<LET x = FN f x\<^sub>l . e\<^sub>l in e, \<rho>, \<kappa>>>
  " |
  Pair: "
    \<lbrakk>
      (\<V>, \<C>) \<Turnstile> (LET x = \<lparr>x\<^sub>1, x\<^sub>2\<rparr> in e); 
      ...
      (\<V>, \<C>) \<Turnstile> \<V> (\<lfloor>e\<rfloor>) \<Rrightarrow> \<kappa>
    \<rbrakk>  \<Longrightarrow>
    (\<V>, \<C>) \<TTurnstile> <<LET x = \<lparr>x\<^sub>1, x\<^sub>2\<rparr> in e, \<rho>, \<kappa>>>
  " |
  Left: "
    \<lbrakk>
      (\<V>, \<C>) \<Turnstile> (LET x = LEFT x\<^sub>p in e); 
      ...
      (\<V>, \<C>) \<Turnstile> \<V> (\<lfloor>e\<rfloor>) \<Rrightarrow> \<kappa>
    \<rbrakk> \<Longrightarrow>
    (\<V>, \<C>) \<TTurnstile> <<LET x = LEFT x\<^sub>p in e, \<rho>, \<kappa>>>
  " |
  Right: "
    \<lbrakk>
      (\<V>, \<C>) \<Turnstile> (LET x = RIGHT x\<^sub>p in e); 
      ...
      (\<V>, \<C>) \<Turnstile> \<V> (\<lfloor>e\<rfloor>) \<Rrightarrow> \<kappa>
    \<rbrakk> \<Longrightarrow>
    (\<V>, \<C>) \<TTurnstile> <<LET x = RIGHT x\<^sub>p in e, \<rho>, \<kappa>>>
  " |
  Send_Evt: "
    \<lbrakk>
      (\<V>, \<C>) \<Turnstile> (LET x = SEND EVT x\<^sub>c x\<^sub>m in e); 
      ...
      (\<V>, \<C>) \<Turnstile> \<V> (\<lfloor>e\<rfloor>) \<Rrightarrow> \<kappa>
    \<rbrakk> \<Longrightarrow>
    (\<V>, \<C>) \<TTurnstile> <<LET x = SEND EVT x\<^sub>c x\<^sub>m in e, \<rho>, \<kappa>>>
  " |
  Recv_Evt: "
    \<lbrakk>
      (\<V>, \<C>) \<Turnstile> (LET x = RECV EVT x\<^sub>c in e); 
      ...
      (\<V>, \<C>) \<Turnstile> \<V> (\<lfloor>e\<rfloor>) \<Rrightarrow> \<kappa>
    \<rbrakk> \<Longrightarrow>
    (\<V>, \<C>) \<TTurnstile> <<LET x = RECV EVT x\<^sub>c in e, \<rho>, \<kappa>>>
  " |
  Always_Evt: "
    \<lbrakk>
      (\<V>, \<C>) \<Turnstile> (LET x = ALWAYS EVT x\<^sub>c in e); 
      (\<V>, \<C>) \<parallel>\<approx> \<rho>; 
      (\<V>, \<C>) \<Turnstile> \<V> (\<lfloor>e\<rfloor>) \<Rrightarrow> \<kappa>
    \<rbrakk> \<Longrightarrow>
    (\<V>, \<C>) \<TTurnstile> <<LET x = ALWAYS EVT x\<^sub>c in e, \<rho>, \<kappa>>>
  " |
  Let_Spawn: "
    \<lbrakk>
      (\<V>, \<C>) \<TTurnstile> <<e, \<rho>, \<kappa>>>;
      (\<V>, \<C>) \<TTurnstile> <<e\<^sub>c, \<rho>, \<kappa>>>
    \<rbrakk> \<Longrightarrow>
    (\<V>, \<C>) \<TTurnstile> <<LET x = SPAWN e\<^sub>c in e, \<rho>, \<kappa>>>
  "
...
lemma flow_over_state_1: "
  (\<V>, \<C>) \<TTurnstile> <<RESULT x, \<rho>, \<langle>x\<^sub>\<kappa>,e\<^sub>\<kappa>,\<rho>\<^sub>\<kappa>\<rangle> # \<kappa>'>> \<Longrightarrow>
  \<rho> x = Some \<omega> \<Longrightarrow>
  (\<V>, \<C>) \<TTurnstile> <<e\<^sub>\<kappa>,\<rho>\<^sub>\<kappa>(x\<^sub>\<kappa> \<mapsto> \<omega>),\<kappa>'>>
"
sorry
