...
inductive paths_correspond :: "control_path ⇒ abstract_path ⇒ bool" where
  ...
  Next: "
    ...
    paths_correspond (π @ [LNxt x]) (path @ [(NLet x, ENext)])
  " |
  Spawn: "
    paths_correspond π path ⟹
    paths_correspond (π @ [LSpwn x]) (path @ [(NLet x, ESpawn)])
  " |
  Call: "
    paths_correspond π path ⟹
...
inductive paths_correspond_mod_chan :: 
  ...
  Chan: "
    ...
    paths_correspond_mod_chan (ℰ, H) (Ch πC xC) (πC @ (LNxt xC) # πSuff) path
  " |
  Sync: "
    paths_correspond πSuffix pathSuffix ⟹
    ℰ (πR @ (LNxt xR) # πSuffix) ≠ None ⟹
    dynamic_built_on_chan_var ρRY c xR ⟹
    ℰ πS = Some (⟨(Let xS (Sync xSE) eSY);ρSY;κSY⟩) ⟹
    ℰ πR = Some (⟨(Let xR (Sync xRE) eRY);ρRY;κRY⟩) ⟹
    {(πS, c, πR)} ⊆ H ⟹
    paths_correspond_mod_chan (ℰ, H) c πS pathPre ⟹
    paths_correspond_mod_chan (ℰ, H) c (πR @ (LNxt xR) # πSuffix) (pathPre @ (NLet xS, ESend xSE) # (NLet xR, ENext) # pathSuffix)
  " 
...
lemma not_static_inclusive_sound: "
  ...
  static_live_traceable F Ln Lx (NLet xC) (static_send_label V e xC) path1 ⟹
