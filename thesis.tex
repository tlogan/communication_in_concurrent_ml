...
indiating how the program identifier transitions to the program identigier of one of
its subprograms.
...
In the case of application, if the applied name is acutally bound to a function
...
to syncrhonize to send on a static channel in any run of a given program.
...
that there may be many threads that attepmt to send on a static channel, but there is at most
...
useful induction hypotheses.  It is often useful to create helper definitions that can be deduced
from premises of the theorem being proved and enable general reasoning across arbitrary programs.
...
in an inductive structure and inaccessble by a predictable number of
...
statically classified as one-shot for a given program and static environment consistent with the
program, then any corresponding dynamic channel is classified as one-shot over any pool that results
from running the program. The theorem for soundness of static fan-out classification states that if
a static channel is statically classified as fan-out for a given program and static environment
consistent with that program, then any corresponding dyanmic channel is classified as fan-out
...
The soundness theorem for static fan-out classification is proved by a few simpler lemmas and the
definitions of static and dynaic fan-out classification.  The three main lemmas state the
...
correspondance between static paths and dynamic paths, which is bijective for the lower
precision analysis. The lemma for soundness of lack of static inclusiveness states that any two
dynamic paths traced by running a programm correspond to statically inclusive static paths. It
follows from a straight forward case analysis of static inclusivity. The lemma for soundness of
lack of static tracebility states that for any dynamic path traced by running a program, there
...
syncrhonization on a sending event, then that synchronization is statically identified as a
...
static traversability and static evalulation over pools, such that information about a point in
the program can be deduced by a fixed number of logical steps regardless of the location of the
program point or the size of the program. Without such generalization, it would be possibe to
...
traversabiliy over pools.
In most cases, it simply states that a subprogram of some semantic element is also statically
traversable. The exception is in the case of  static traversability over a non-empty stack, where
there is an additional condition that the transition from a result site to the program identifier
...
static program, which would otherwise only be decucible by a varying number of logical steps
...
environments in order to look up the control flow in the application case. The static environment
...
the generalized versions of static evaluation are designed to preserve static environments across
...
of soundness of lack of static traceability over pools, which contains the generalized premise of
...
soundness of a name in a sending event of a syncrhonization not statically binding to a static
channel value, and the lemma for soundess of a name in any state's environment not statically binding
to a static value.  Since only send sites are relevant the soundness of lack of static reachability is
...
Both the soundness of a name in a sending event of a syncrhonization not statically binding
to a static
channel value, and the soundess of a name in any state's environment not statically binding
...
The lemma for soundness of lack of static rechability relies on the a reformulation of
static rechability that inducts on the left program.
The definition of static reachability is syntax-directed in order to potray a clear connection to
...
induction needs to enable unraveling the goals from the end to the begininng of the program,
maintaining the intial program state in context for each subgoal .
...
The soundness of lack of reachbility follows the the definitions a generalized form of soundness over
pools.
...
The soudness over pools follows from the lemma that the leftward static reachability implies the
rightward (and syntax-directed) static reachability, and the equivalence between star and the
leftward star.  It relies on induction of the leftward star and constructs the static rechability
...
In the whole static program used in the previous analysis, a spawning transition connects a child
thread to the rest of the program.  For a trimmed static program, it may be clear the channel of
interest is not created until after the spawn point, so there is not need to include the spawning
transition.  However, later on in the program it may become apparent that the channel of interest
is sent via another channel to that spawned thread.  Since there is no spawning transition already
connecting that thread to the trimmed static program, a transition with a sending mode is used
between the send site and the receive site of syncrhonization. Modes for typical control flow of
...
Static traversability is similar to that of the previous analysis.  However, it must additionally
consider transitions with the sending mode. 
...
An entry function contains the names built on the channel that might be live at then entrance
...
exit functions. The entry function maps a program point to a set of variables built on
the given channel, if those variables are live at the entry of that program identifier.
...
\begin{lstlisting}[language=logic, escapechar=\%]
  predicate static_live_chan of
    ...
    (%$\forall$% exit x entr e' e_c x_c env_a .
      if
        (exit (NLet x) - {x}) %$\subseteq$% entr (NLet x) and
        entr (top_program_id e') %$\cup$% entr (top_program_id e_c) %$\subseteq$% exit (NLet x) and
...
The static traceability for the higher precision analysis states that an entire static path can be
trace through some static program and is live with respect to some entry and and exit functions.
...
The communication topology classifications are described using the liveness properties, but are
otherwise similar to the lower precision classifications.
...
To prove soundess of the communication topology classification, it should be possible to use
...
the trimmed static programs, which might also contain sending transitions, instead of the dynamic
paths spawning transitions.  The correspondance between these dynamic paths and static paths
is not bijective, as it is for the lower precision analysis. However, finding a satisfactory
correspondance for each dynamic and static path is critical for proving soundness.
...
analysis. These reasoning strategies are demonstrated by the theorem for soudness of
...
The theorem for soudness of one-shot classification depends on
...
Additionally the soudness theorem follows from the soundness of lack of static traceability, the
soundness of lack of static inclusiveness, and the soundness of a program site not being a
sending site. 
...
correlation was straight forward. There was only one possible static path to choose for it
