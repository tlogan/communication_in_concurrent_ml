...
program_ided with a program point.  A sequence of program points ordered in a valid execution
...
program_id whose state has the next expression and the environment updated
with the unit value bound to
the let binding name, and the original continuation stack. For the other, the leaf path
extended with an program_id indicating a spawning transition.  Its state has the spawned
...
there is an additional condition that the transition from a result point to the program_id of
the continuation program exists in the static program. This information is consistent with static
...
\begin{lstlisting}[language=logic, escapechar=\%]
  ...
  predicate static_one_to_one of static_environment -> program -> name -> bool:
    ...
    (%$\forall$% prog_a entr exit x_c env_a e .
      if
        every_two
          (static_live_traceable prog_a entr exit (NLet x_c) (static_send_site env_a e x_c))
          noncompetitive and 
        every_two
          (static_live_traceable prog_a entr exit (NLet x_c) (static_recv_program_id env_a e x_c))
          noncompetitive and
          static_live_chan env_a entr exit x_c e and
          static_traversable env_a prog_a e
      then
