  ...
  \begin{lstlisting}[style=codestyle1, escapechar=\%]
    ...
    inductive concur_step ::
      ...
      Seq_Step:
        ...
        concur_step
          (trpl, ys)
          (trpl(pi;;(LNext x) %$\mapsto$% (Stt e (env(x %$\mapsto$% v)) k), ys) |
      Seq_Step_Up: 
        leaf trpl pi %$\Longrightarrow$%
        trpl pi = Some (Stt (Let x b e) env k) %$\Longrightarrow$%
      ...
      Let_Chan: 
        leaf trpl pi %$\Longrightarrow$%
        trpl pi = Some (Stt (Let x Chan e) env k) %$\Longrightarrow$%
        concur_step
          (trpl, ys)
          (trpl(pi;;(LNext x) %$\mapsto$%
            (Stt e (env(x %$\mapsto$% (VChan (Ch pi x)))) k)), ys) |
      Let_Spawn:
        leaf trpl pi %$\Longrightarrow$%
        trpl pi = Some (Stt (Let x (Spawn ec) e) env k) %$\Longrightarrow$%
        concur_step
          (trpl, ys)
          (trpl(
            pi;;(LNext x) %$\mapsto$% (St e (env(x %$\mapsto$% VUnit)) k), 
            pi;;(LSpawn x) %$\mapsto$% (St ec env []), ys) |
      Let_Sync:
        leaf trpl pis %$\Longrightarrow$%
        trpl pis = Some (Stt (Let xs (Sync xse) es) envs ks) %$\Longrightarrow$%
        envs xse = Some (VClsr (Send_Evt xsc xm) envse) %$\Longrightarrow$%
        leaf trpl pir  %$\Longrightarrow$%
        trpl pir = Some (Stt (Let xr (Sync xre) er) envr kr) %$\Longrightarrow$%
        envr xre = Some (VClsr (Recv_Evt xrc) envre) %$\Longrightarrow$%
        envse xsc = Some (VChan c) %$\Longrightarrow$%
        envre xrc = Some (VChan c) %$\Longrightarrow$%
        envse xm = Some vm %$\Longrightarrow$%
        concur_step
          (trpl, ys)
          (trpl(
            pis;;(LNext xs) %$\mapsto$% (Stt es (envs(xs %$\mapsto$% VUnit)) ks), 
            pir;;(LNext xr) %$\mapsto$% (Stt er (envr(xr %$\mapsto$% vm)) kr)), 
            ys %$\cup$% {(pis, c, pir)})
