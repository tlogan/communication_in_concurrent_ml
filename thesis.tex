...
definition is_recv_path of trace_pool -> chan -> control_path -> bool:
only
...
definition static_recv_label of abstract_env -> exp -> var -> label -> bool:
only
...
definition singular of abstract_path -> abstract_path -> bool:
only
(∀ path .
...
definition static_reachable_over_stack of exp -> contin list -> bool:
only
(∀ e0 .
...
For the live analysis it is necessary to track any variable that is bound to a channel of interest,
since any use of that variable would indicate a use of the channel. The predicate
static_built_on_chan states that a variable is bound to some structure containing a channel. 

inductive 
  static_built_on_chan :: "abstract_env ⇒ label_map ⇒ var ⇒ var ⇒ bool"
where
  Chan: "
    ⟦
      ^Chan x⇩c ∈ V x 
  ...
  Abs: "
    ^Abs f x⇩p e⇩b ∈ V x ⟹ 
    ¬ Set.is_empty (Ln (top_label e⇩b) - {x⇩p}) ⟹
    static_built_on_chan V Ln x⇩c x
  " 
