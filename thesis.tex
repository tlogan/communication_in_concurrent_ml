...
\textit{Concurrent ML} is a particularly elegant programing language for concurrency.
...
formulated in a higher order logic (HOL)\cite{}. Predicates and functions may be higher order. That is, they
...
A proof is a sequence of manipulations from axioms to the claim.  
...
allows the you to focus on the manipulation rule, rather than remembering and gathering up all the theorems that are
...
over infinite ranges. Infinite ranges may be defined as inductive datatypes, similar to Standard ML.
...
Additionaly, these are the only cases that can hold for the predicate. Therefore, the predicate applied to some free
...
static communication, soundness of communication, helper definitions, and theorems. 
...
described by the dynamic semantics of the program. The analysis is computable if there is an algorithm that
determines values sufficient for the analysis to hold from an input program.
...
many-to-one, one-to-one, one-shot, or one-sync. All four classification relations are defined in terms of how any two paths   
  ...
  * second: path2 path1 .
  ...
  predicate oneSync: tm -> chan -> bool where
    intro: t$_0$ c .
      star dynamicEval [[] -> (Stt t$_0$ [->] [])] {} pool comm,
      forEveryTwo (isSendPath pool c) (op =),
      forEveryTwo (isRecvPath pool c) (op =)
...
\begin{lstlisting}[language=logic, mathescape]
  predicate staticOneSync: static_value_map -> term -> name -> bool where
    intro: staticEnv staticComm t graph n$_c$ .
      ...
      forEveryTwo (staticTraceable graph (termId t) (staticRecvId staticEnv t n$_c$)) singular,
...
\begin{lstlisting}[language=logic, mathescape]
  ...
  where
    seq: l l' graph exit entr . 
      ...
      $\neg$ (exit l = {}),
      $\neg$ (entr l' = {})
    $\vdash$ staticLiveFlow graph entr exit (l, MNxt, l')
...
\begin{lstlisting}[language=logic, mathescape]
  ...
  * return: path staticPath n .
  ...
  where
    seq: pool path$_c$ n$_c$ path$_{sfx}$ stt staticPath comm .
  ...
  * send: pool path$_r$ n$_r$ path$_{sfx}$ stt  path$_s$ n$_s$ n$_{se}$ t$_{sy}$ env$_{sy}$ stack$_{sy}$
    ...
    $\vdash$ pathsCorrespondModChan pool comm c
