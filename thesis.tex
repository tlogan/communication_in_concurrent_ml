...
\textit{Concurrent ML} with the \lstinline{then} combinator described in their work on
...
the \lstinline{then} combinator, rather than duplicating code with the addition of stronger
isolation. When the event constructed by the \lstinline{then} combinator is is synchronized on,
either all of its constituent events and abstractions evaluate in isolation, or none evaluates.
...
One way to lower synchronization and communication costs is to specialize the implementation
for channels that have no more than one process ever sending or receiving on them.

----Edit From Here----

Channels known to have only one sender or one receiver can have lower communication costs than those with arbitrary number of senders and arbitrary number of receivers, since some of the
