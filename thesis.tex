...
\begin{lstlisting}[language=logic, mathescape]
  ...
  only
  (\forall pool path n env n_k p_k env_k stack' v comm .
    if
      leaf pool path,
      pool path = Some
        (
          Stt (Rslt n) env ((Ctn n_k p_k env_k) # stack')
        ),
      env n = Some v
    then
      dynamic_eval
        ...
        comm
        (pool(
          path @ [DRtn n] -> (Stt p_k env_k(n_k -> v) stack'))
        )
        comm
  ),
  ...
  (\forall pool path n b p' env stack p_u env_u comm .
    if 
      leaf pool path,
      pool path = Some (Stt (Bind n b p') env stack),
  ...
  (\forall pool path n p_c p' env stack comm .
    if 
      leaf pool path, 
  ...
  (\forall pool path_s path n$_s$ n$_se$ p_s env_s stack_s n$_sc$ n_m env_se
    path_r n$_r$ n$_r$e p$_r$ env_r stack_r n$_r$c env_re c comm .
    if 
      leaf pool path_s,
      pool path_s = Some (Stt
        (Bind n$_s$ (Sync n$_s$e) p_s) env_s stack_s),
      env_s n$_s$e = Some (VAtm
        (SendEvt n$_s$c n_m) env_se),
      leaf pool path_r,
      pool path_r = Some (Stt
        (Bind n$_r$ (Sync n$_r$e) p$_r$) env_r stack_r),
      env_r n$_r$e = Some (VAtm
...
\begin{lstlisting}[language=logic, mathescape]
  predicate is_send_path of
    ...
    (\forall pool path n n_e p' env stack n$_s$c n_m env_e c.
      if
        ...
        env n_e = Some (VAtm (SendEvt n$_s$c n_m) env_e), 
  ...
  predicate is_recv_path of pool -> chan -> dynamic_path -> bool:
    ...
    (\forall poo path n n_e p' env stack n$_r$c env_e c .
      then
        pool path = Some (Stt (Bind n (Sync n_e) p') env stack),
        env n_e = Some (VAtm (RecvEvt n$_r$c) env_e),
