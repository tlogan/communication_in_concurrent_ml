...
The dynamic semantics described how programs evaluate to values.  A history of execution is
...
The evaluation of expressions for application, and matching result in calling, which leads 
...
(%$\forall$% pool path x env x_k e_k env_k stack' v convo .
  ...
  then
    concurrent_eval
      pool convo
      (pool(path @ [DRtn x] -> (Stt e_k env_k(x_k -> v) stack'))) convo) and
...
(%$\forall$% pool path x b e' env stack v .
  if 
    ...
    seq_eval b env v
  then
    concurrent_eval pool convo (pool(path @ [DNxt x] -> (Stt e (env(x -> v)) stack))) convo)
...
(%$\forall$% pool path x b e' env stack e_u env_u convo .
  ...
  then
    concurrent_eval
      pool convo
      (pool(path @ [DCll x] -> (Stt e_u env_u ((Ctn x e' env) # stack)) convo)
...
(%$\forall$% pool path x e' env stack .
  ...
  then
    concurrent_eval
      pool convo 
      (pool(path @ [DNxt x] -> (Stt e' (env(x -> (VChn (Chan path x)))) stack))) convo) and
...
(%$\forall$% pool path x e_c e' env stack convo .
  ...
  then
    ...
    (pool(
        ...
        path @ [DSpwn x] -> (Stt e_c env []))) convo) and
...
(%$\forall$% pool path_s path x_s x_se e_s env_s stack_s x_sc x_m env_se
  ...
  then
    concurrent_eval
      pool convo
      (pool(
          path_s @ [DNxt x_s] -> (Stt e_s (env_s(xs -> VUnt)) stack_s), 
          path_r @ [DNxt x_r] -> (Stt e_r (env_r(x_r -> v_m))stack_r))) 
...
predicate static_traversable_val of static_environment -> transition_set -> dynamic_value -> bool:
...
(%$\forall$% path path_a x .
  ...
  then
    paths_correspond (path @ [DNxt x]) (path_a @ [(NLet x, MNxt)])) and

(%$\forall$% path path_a x .
  if
    paths_correspond path path_a
  then
    paths_correspond (path @ [DSpwn x]) (path_a @ [(NLet x, MSpwn)])) and

(%$\forall$% path path_a x .
  if
    paths_correspond path path_a
  then
    paths_correspond (path @ [DCll x]) (path_a @ [(NLet x, MCll)])) and

(%$\forall$% path path_a x .
  if
    paths_correspond path path_a
  then
    paths_correspond (path @ [DRtn x]) (path_a @ [(NResult x, MRtn)]))


predicate paths_correspond_mod_chan of 
  pool -> conversation -> channel -> dynamic_path -> static_path -> bool:
