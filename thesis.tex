...
description of channel liveness in terms of entry maps and exit maps.  The entry map maps a
program point to a set of variables built on the given channel, if those variables are live at
the entry of that program point.  The exit map maps a program point to a set of variables built
on the given channel, if those variabes are live at the exit of that program point.
...
(∀ exit x entr e_l x_l e_r x_r env_a x_c x_s e' .
  if 
    (exit (NLet x) - {x}) ⊆ entr (NLet x) and
    (Ln (top_label e_l) - {x_l}) ⊆ entr (NLet x) and
    (Ln (top_label e_r) - {x_r}) ⊆ entr (NLet x) and
...
\begin{lstlisting}[style=codestyle1, escapechar=\%]
  ...
  Let_Abs:
      ...
      (exit (NLet x) - {x}) %$\cup$%
        (Ln (top_label eb) - {xp}) = entr (NLet x) %$\Longrightarrow$%
  ...
  Let_Case:
      ...
      (Ln (top_label er) - {xr}) = entr (NLet x) %$\Longrightarrow$%
