...
synchronizes and sends on a given channel.

Whether two attempts to synchronize on a channel are competitive or not can be determined by
...
from running the program.  The lemmas for soundness of fan out, fan in, and one to one follow
similar patterns. 
...
The most difficult aspect of formalizing a theory is in developing adequate definitions that
enable formal reasoning.  It is often possible to define a single semantics in multiple ways.
...
We need a stronger induction hypothesis that enables direct access to properties necessary to
prove the theorem.  In addition to the original predicate defined in terms of the expression,
I define predicates in terms of the pool, environment and stack.  I prove that a particular use
of these predicates imply the original use of the the expression predicate.  Some parts of the 
predicate definitions allow direct access to properties that would otherwise be deeply nested
in an inductive structure and difficult to access.  The other aspects of these predicate
definitions are simply to prove that the definition is a generalization of that expression
predicate.    
-------
