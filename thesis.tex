...
\begin{lstlisting}[language=logic, mathescape]
  predicate staticSendId:
    static_value_map -> term -> name -> term_id -> bool 
  where
...
The static traceability means that a static path with a given starting term identifier, and ending
condition, can be traced by traversing the flows in a graph.
The empty path is statically traceable if the starting term identifier meets the ending condition.
...
soundness of static traceability, the soundness of static inclusiveness, and
...
in the definition of static traceability.
...
The lemma for soundness of static traceability follows from the generalized definitions
of static flows acceptance, the definition of static traceability, and the preservation
...
The static flow liveness checks if a flow is live across call and return boundaries,
...
Additionally, for the intraprocedural flows, it simply lifts the entry and exit function information.
...
In such a case, the live flow is retained from the caller to within the loop function's body and from the result
of the body back to the caller's sequenced term, 
even though the steps in the loop may not be live according to the entry function in static channel liveness.

\begin{lstlisting}[language=logic, mathescape]
  predicate staticFlowLive:
  ...
  where
    ...
    seq: l l' graph exit entr . 
      ...
      $\neg$ (exit l = {}),
      $\neg$ (entr l' = {})
    $\vdash$ staticFlowLive graph entr exit (l, MSeq, l')

  * spawn: l l' graph exit entr .
      (l, MSpwn, l') $\in$ graph, 
      $\neg$ (exit l = {}),
      $\neg$ (entr l' = {})
    $\vdash$ staticFlowLive graph entr exit (l, MSpwn, l')

  * call: l l' graph exit entr .
      (l, MCll, l') $\in$ graph,
      ($\neg$ (exit l = {})) $\vee$ ($\neg$ (entr l' = {}))
    $\vdash$ staticFlowLive graph entr exit (l, MCll, l')

  * return: l l' graph entr exit .
      (l, MRtn, l') $\in$ graph,
      $\neg$ (entr l' = {})
    $\vdash$ staticFlowLive graph entr exit (l, MRtn, l')

  * send: n$_s$ n$_e$ n$_r$ graph entr exit .
      ((NBnd n$_s$), MSend n$_e$, (NBnd n$_r$)) $\in$ graph, 
      {n$_e$} $\subseteq$ (entr (NBnd n$_s$))
    $\vdash$ staticFlowLive graph entr exit
      ((NBnd n$_s$), MSend n$_e$, (NBnd n$_r$))
\end{lstlisting}
...
\begin{lstlisting}[language=logic, mathescape]
   ...
   * snoc: graph entr exit start middle path isEnd mode. 
       ...
       (isEnd end),
       staticFlowLive graph entr exit (middle, mode, end) 
...
\lstinline[mathescape]{dynamicBuiltOnChanState (Stt t' env' stack') (Ch path$_c$ n$_c$)} in the soundess of static traceability.
...
the paths corresponding modulo a channel and static traceability.
