...
\begin{lstlisting}[language=logic, mathescape]
  ...
  only
  (\forall pool path n n$_e$ p' env stack n$_rc$ env$_e$ c .
    then
      pool path = Some (Stt (Bind n (Sync n$_e$) p') env stack),
      env n$_e$ = Some (VAtm (RecvEvt n$_rc$) env$_e$),
      env$_e$ n$_rc$ = Some (VChn c)
    then
  ...
  (\forall staticEnv n$_e$ n staticComm  t'.
    if
      (\forall n$_sc$ n$_m$ n$_c$ . 
        if
          (SAtm (SendEvt n$_sc$ n$_m$)) \in staticEnv n$_e$, 
...
\begin{lstlisting}[language=logic, mathescape]
  ...
  type termId_map = termId -> name set
\end{lstlisting}
...
\begin{lstlisting}[language=logic, mathescape]
  ...
  (\forall staticEnv graph env .
    if
      (\forall n v . if env n = Some v then
        staticFlowsAccept_val staticEnv graph v
      )
    then
      staticFlowsAccept_env staticEnv graph env
  ) 

  predicate staticFlowsAccept_stack
  of static_value_map -> graph -> name -> continuation list -> bool:
  only
  (\forall staticEnv graph y .
    staticFlowsAccept_stack staticEnv graph y []
  ),
  ...
  (\forall y p graph staticEnv graph env stack n env .
    if 
      ...
      staticFlowsAccept_env staticEnv graph env,
      staticFlowsAccept_stack staticEnv graph (resultName e) stack 
    then 
      staticFlowsAccept_stack staticEnv graph y ((Ctn n p env) # stack)
  )

  predicate staticFlowsAccept_pool of
...
\begin{lstlisting}[language=logic, mathescape]
  predicate staticReachable_forward of term -> term -> bool:
  only
  (\forall t0 .
    staticReachable_forward t0 t0
  ),
  (\forall t0 n t$_c$ t' .
    if 
      staticReachable_forward t0 (Bind n (Spwn t$_c$) t')
    then
      staticReachable_forward t0 t$_c$
  ),
  (\forall t0 n n$_s$ n$_l$ t$_l$ n$_r$ t$_r$ t' .
    if
      staticReachable_forward t0 (Bind n (Case n$_s$ n$_l$ t$_l$ n$_r$ t$_r$) t')
    then
      staticReachable_forward t0 t$_l$
  ),
  (\forall t0 n n$_s$ n$_l$ t$_l$ n$_r$ t$_r$ t' .
    if
      staticReachable_forward t0 (Bind n (Case n$_s$ n$_l$ t$_l$ n$_r$ t$_r$) t')
    then
      staticReachable_forward t0 t$_r$
  ),
  (\forall t0 n n$_f$ n$_p$ t$_b$ t' .
    if
      staticReachable_forward t0 (Bind n (Atom (Fun n$_f$ n$_p$ t$_b$)) t')
    then
      staticReachable_forward t0 t$_b$
  ),
  (\forall t0 n n$_f$ n$_p$ t$_b$ t' .
    if
      staticReachable_forward t0 (Bind n c t')
    then
      staticReachable_forward t0 t'
  )

  predicate staticReachable_over_atom of term -> atom -> bool:
  only
  (\forall t0 n$_c$ n$_m$ .
    staticReachable_over_atom t0 (SendEvt n$_c$ n$_m$)
  ),
  (\forall t0 n$_c$ .
    staticReachable_over_atom t0 (RecvEvt n$_c$)
  ),
  (\forall t0 n1 n2 .
    staticReachable_over_atom t0 (Pair n1 n2)
  ),
  (\forall t0 n$_l$ .
    staticReachable_over_atom t0 (Lft n$_l$)
  ),
  (\forall t0 n$_r$
    staticReachable_over_atom t0 (Rht n$_r$)
  ),
  (\forall t0 t$_b$ n$_f$ n$_p$ t$_b$ . 
    if
      staticReachable_forward t0 t$_b$ 
    then
      staticReachable_over_atom t0 (Fun n$_f$ n$_p$ t$_b$)
  )

  predicate staticReachable_val of term -> dynamic_value -> bool:
  only
  (\forall t0 .
    staticReachable_over_val t0 VUnt
  ),
  (\forall t0 c .
    staticReachable_over_val t0 (VChn c)
  ),
  (\forall t0 t env .
    if
      staticReachable_over_atom t0 t, 
      staticReachable_over_env t0 env
    then
      staticReachable_over_val t0 (VAtm t env)
  )
    ...
    if
      (\forall n v .
        if
          env n = Some v
        then
          staticReachable_over_val t0 v
      )
    then
      staticReachable_over_env t0 env
  )

  predicate staticReachable_over_stack
  of term -> continuation list -> bool:
  only
  (\forall t0 .
    staticReachable_over_stack t0 []),
  (\forall t0 t$_k$ env$_k$ stack' .
    if
      staticReachable_forward t0 t$_k$, 
      staticReachable_over_env t0 env$_k$,
      staticReachable_over_stack t0 stack' 
    then
    staticReachable_over_stack t0 ((Ctn n$_k$ t$_k$ env$_k$ # stack'))
  )
  ...
  (\forall n t' graph staticEnv n$_s$e .
    if
      ...
      (\forall n$_sc$ n$_m$ n$_c$ y.
        if
          (SAtm (SendEvt n$_sc$ n$_m$)) \in staticEnv xSE, 
          (SChn n$_c$) \in staticEnv n$_sc$,
...
\includegraphics[width=1.3\textwidth, left]{cml-graph-lp.pdf}
...
\begin{lstlisting}[language=logic, mathescape]
  ...
  of static_value_map -> termId_map -> termId_map -> name -> term -> bool:
  only
...
\begin{lstlisting}[language=logic, mathescape]
  ...
  graph -> termId_map -> termId_map -> flow -> bool:
  only
...
\begin{lstlisting}[language=logic, mathescape]
  predicate staticTraceable
  of static_value_map -> graph -> termId_map -> termId_map ->
...
\begin{lstlisting}[language=logic, mathescape]
  ...
  only
  (\forall pool path$_c$ n$_c$ path$_sfx$ stt staticPath comm .
    if
      pool (path$_c$ @ (DNxt n$_c$) # path$_sfx$) = Some stt,
      pathsCorrespond ((DNxt n$_c$) # path$_sfx$) staticPath
    then
      pathsCorrespondModChan
        (pool, comm) (Chan path$_c$ n$_c$)
        (path$_c$ @ (DNxt n$_c$) # path$_sfx$) staticPath
  ),
  (\forall pool path$_r$ n$_r$ path$_sfx$ stt  path$_s$ n$_s$ n$_se$ t$_sy$ env$_sy$ stack$_sy$
    n$_r$e t$_ry$ env$_ry$ stack$_ry$ c$_c$ comm c staticPath$_re$ staticPath$_sfx$ . 
    if
      pool (path$_r$ @ (DNxt n$_r$) # path$_sfx$) = Some stt, 
      pool path$_s$ = Some (Stt (Bind n$_s$ (Sync n$_se$) t$_sy$) env$_sy$ stack$_sy$),
      pool path$_r$ = Some (Stt (Bind n$_r$ (Sync n$_r$e) t$_ry$) env$_ry$ stack$_ry$),
      {(path$_s$, c$_c$, path$_r$)} \subseteq comm, 
      dynamicBuiltOnChanVar env$_ry$ c n$_r$, 
      pathsCorrespondModChan pool comm c path$_s$ staticPath$_pre$,
      pathsCorrespond path_$sfx$ staticPath$_sfx$
    then
      pathsCorrespondModChan pool comm c
        (path$_r$ @ (DNxt n$_r$) # path$_sfx$)
        (staticPath$_pre$ @ (NBnd n$_s$, ESend n$_se$) # (NBnd n$_r$, MNxt) # staticPath$_sfx$))
\end{lstlisting}
...
\begin{lstlisting}[language=logic, mathescape]
  ...
  (\forall pool path$_r$ n$_r$ path$_sfx$ stt  path$_s$ n$_s$ n$_s$e t$_sy$ env$_sy$ stack$_sy$
    n$_r$e p$_r$y env$_ry$ stack$_ry$ c$_c$ comm c staticPath$_re$ staticPath$_sfx$ . 
    if
      pool (path$_r$ @ (DNxt n$_r$) # path$_sfx$) = Some stt, 
      pool path$_s$ = Some (Stt (Bind n$_s$ (Sync n$_s$e) t$_sy$) env$_sy$ stack$_sy$),
      pool path$_r$ = Some (Stt (Bind n$_r$ (Sync n$_r$e) t$_ry$) env$_ry$ stack$_ry$),
      {(path$_s$, c$_c$, path$_r$)} \subseteq comm, 
      dynamicBuiltOnChanVar env$_ry$ c n$_r$, 
      pathsCorrespondModChan pool comm c path$_s$ staticPath$_pre$,
      pathsCorrespond path$_sfx$ staticPath$_sfx$
    then
      pathsCorrespondModChan pool comm c
        (path$_r$ @ (DNxt n$_r$) # path$_sfx$)
        (staticPath$_pre$ @ (NBnd n$_s$, ESend n$_s$e) # (NBnd n$_r$, MNxt) # staticPath$_sfx$)
  )
  lemma staticTraceable_complete: 
