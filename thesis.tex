...
\lstdefinelanguage{normal_lang}{
  morekeywords={
    ...
    mk_chn, sync, fst, snd, lft, rght,
    spawn, case, of, send_evt, recv_evt
  }
}
...
and there is an algorithm to compute sufficient parameters from a program.
...
that point. atomic terms are not simplified.  Instead, atoms are evaluated to
...
\begin{lstlisting}[language=logic]
  ...
  datatype dynamic_value = 
    VUnt | VChn channel | VPrm atom (name -> dynamic_value option)

  type environment =
    name -> dynamic_value option
  \end{lstlisting}

The evaluation of some terms results in sequencing, meaning there is no coordination
with other threads, and there is no backtracking. These terms are the
unit literal, atoms, pairs, and first and second projections. The evaluation depends only
...
\begin{lstlisting}[language=logic, mathescape]
  predicate seq_eval of complex -> environment -> dynamic_value -> bool:
    ...
    (\forall p env .
        seq_eval (Atom p) env (VPrm p env)),
...
The evaluation of terms for application, and matching result in calling, which leads to
...
The terms are evaluated to a called program, and a new environment that will
...
\begin{lstlisting}[language=logic, mathescape]
  predicate call_eval of complex -> env -> term -> env -> bool:
    ...
    (\forall env n$_f$ n$_f$' n$_p$ p$_b$ env_l n$_a$ v .
      if 
        env n$_f$ = Some (VPrm (Fun n$_f$' n$_p$ p$_b$) env_l); 
        env_l n$_a$ = Some v
      then
        call_eval (App n$_f$ n$_a$) env p$_b$ (
          env_l(n$_f$' -> (VPrm (Fun n$_f$' n$_p$ p$_b$) env_l), n$_p$ -> v)))
  \end{lstlisting}
...
environment is updated with the name bound to the value of the term. Each time a
...
Additionally, it generates a another state consisting of the spawning term's child
...
program point whose state has the next term and the environment updated
...
\begin{lstlisting}[language=logic, mathescape]
  predicate dynamic_eval of
    ...
    (\forall pool path n env n_k p_k env_k stack' v convo .
      if
        leaf pool path,
        pool path = Some (Stt
          (Rslt n) env ((Ctn n_k p_k env_k) # stack')),
        env n = Some v
      then
        dynamic_eval
          pool convo
          (pool(path @ [DRtn n] -> (Stt p_k env_k(n_k -> v) stack')))
          convo),
    ...
    (\forall pool path n b p' env stack v .
      ...
      then
        dynamic_eval
          pool convo
          (pool(path @ [DNxt n] -> (Stt p (env(n -> v)) stack)))
          convo),

    (\forall pool path n b p' env stack p_u env_u convo .
      if 
        leaf pool path,
        pool path = Some (Stt (Bind n b p') env stack),
        call_eval b env p_u env_u
      then
        dynamic_eval
          pool convo
          (pool(path @ [DCll n] ->
            (Stt p_u env_u ((Ctn n p' env) # stack))
          convo),
    ...
    (\forall pool path n p_c p' env stack convo .
      if 
        leaf pool path, 
        pool path = Some (Stt (Bind n (Spwn p_c) p') env stack)
      then
        dynamic_eval
        pool convo 
        (pool(
    ...
    (\forall pool path_s path n$_s$ n$_s$e p_s env_s stack_s n$_s$c n_m env_se
      path_r n$_r$ n$_r$e p$_r$ env_r stack_r n$_r$c env_re c convo .
      if 
        leaf pool path_s,
        pool path_s = Some (Stt
          (Bind n$_s$ (Sync n$_s$e) p_s) env_s stack_s),
        env_s n$_s$e = Some (VPrm
          (SendEvt n$_s$c n_m) env_se),
        leaf pool path_r,
        pool path_r = Some (Stt
          (Bind n$_r$ (Sync n$_r$e) p$_r$) env_r stack_r),
        env_r n$_r$e = Some (VPrm
          (RecvEvt n$_r$c) env_re),
        env_se n$_s$c = Some (VChn c),
        env_re n$_r$c = Some (VChn c), 
        env_se n_m = Some v_m
      then
        dynamic_eval
          pool convo
          (pool(
              path_s @ [DNxt n$_s$] ->
                (Stt p_s (env_s(n_s -> VUnt)) stack_s), 
              path_r @ [DNxt n$_r$] ->
                (Stt p$_r$ (env_r(n$_r$ -> v_m))stack_r))) 
...
function abstractions.  Uncovering the the control structures depends on resolving terms
  ...
  predicate static_eval of
    ...
    (\forall n$_c$ n_m static_env n static_convo p' .
      if
        ...
        static_eval static_env static_convo p' 
      then
        static_eval static_env static_convo (Bind n (Atom (SendEvt n$_c$ n_m)) p')),
    ...
    (\forall n$_c$ static_env n static_convo p' . 
      if 
        ...
        static_eval static_env static_convo p'
      then
        static_eval static_env static_convo (Bind n (Atom (RecvEvt n$_c$)) p')),
    ...
    (\forall n1 n2 static_env n static_convo p' .
      if
        ...
        static_eval static_env static_convo p'
      then
        static_eval static_env static_convo (Bind n (Atom (Pair n_1 n_2)) e)),
    ...
    (\forall n$_s$ static_env n static_convo p' .
      if
        ...
        static_eval static_env static_convo p' 
      then
        static_eval static_env static_convo (Bind n (Atom (Lft n$_s$)) p')),
    ...
    (\forall n$_s$ static_env n static_convo p' .
      ...
      then
        static_eval static_env static_convo (Bind n (Atom (Rght n$_s$)) p')),
    ...
    (\forall n$_f$ n$_p$ p$_b$ static_env static_convo n p' .
      if
        ...
        static_eval static_env static_convo p'
      then
        static_eval static_env static_convo (Bind n (Atom (Fun n$_f$ n$_p$ p$_b$)) p')),
...
\begin{lstlisting}[language=logic, mathescape]
  predicate static_reachable of term -> term -> bool:
    ...
    (\forall p .
      static_reachable p e),

    (\forall p_c p_z n p' . 
      if
        static_reachable p_c p_z
      then
        static_reachable (Bind n (Spwn p_c) p') p_z),

    (\forall p$_l$ p_z n n$_s$ n$_l$ n$_r$ p$_r$ p' . 
      if
        static_reachable p$_l$ p_z
      then
        static_reachable (Bind n (Case n$_s$ n$_l$ p$_l$ n$_r$ p$_r$) p') p_z),

    (\forall p$_r$ p_z n n$_s$ n$_l$ p$_l$ n$_r$ p' . 
      if
        static_reachable p$_r$ p_z
      then
        static_reachable (Bind n (Case n$_s$ n$_l$ p$_l$ n$_r$ p$_r$) p') p_z),

    (\forall p$_b$ p_z n n$_f$ n$_p$ p$_b$ p' . 
      if
        static_reachable p$_b$ p_z
      then
        static_reachable (Bind n (Atom (Fun n$_f$ n$_p$ p$_b$)) p') p_z),

    (\forall p' p_z n b . 
      if
        static_reachable p' p_z
      then
        static_reachable (Bind n b p') p_z)
  \end{lstlisting}
...
traversable transitions for the sequenced term.  For binding to function abstraction, the
traversable transitions are also traversable transitions for the inner term of the
function abstraction.  For binding to spawning, the spawning transition from the top term
...
\begin{lstlisting}[language=logic, mathescape]
  predicate static_traversable_val of
    ...
    (\forall static_env graph p$_b$ env n$_f$ n$_p$ .
      ...
      then
        static_traversable_val
          static_env n$_f$ (VPrm (Fun n$_f$ n$_p$ p$_b$) env)),
...
\begin{lstlisting}[language=logic, mathescape]
  fun abstract of dynamic_value -> static_value:
    ...
    (\forall atom env .
      abstract (VPrm atom env) = SPrm atom)
...
\begin{lstlisting}[language=logic, mathescape]
  predicate static_traversable of static_value_map -> graph -> term -> bool:
    ...
    (\forall n p' graph static_env n$_s$e .
      if
        (NBind n , MNxt, top_program_id p') \in graph,
        (\forall n$_s$c n_m n$_c$ y.
          if
            (SPrm (SendEvt n$_s$c n_m)) \in static_env xSE, 
            (SChn n$_c$) \in static_env n$_s$c,
            static_recv_program_id static_env p' n$_c$ (NBind y)
          then
            (NBind x, ESend n$_s$e, NBind y) \in F),
        static_traversable static_env graph p'
      then
        static_traversable static_env graph (Bind n (Sync n$_s$e) p')),
...
\begin{lstlisting}[language=logic, mathescape]
  theorem static_one_shot_sound:
    \forall e0 pool convo static_env static_convo n$_c$ path_x . 
      if
        star dynamic_eval [[] -> (Stt e0 [->] []] {} pool convo,
        static_eval static_env static_convo e0,
        static_one_shot static_env e0 n$_c$
  ...
  predicate paths_correspond_mod_chan of 
    ...
    (\forall pool path_r n$_r$ path_sfx stt  path_s n$_s$ n$_s$e p_sy env_sy stack_sy
      ...
      if
        ...
        {(path_s, c_c, path_r)} \subseteq convo, 
        dynamic_built_on_chan_var env_ry c n$_r$, 
        paths_correspond_mod_chan pool convo c path_s static_path_pre,
        paths_correspond path_sfx static_path_sfx
      then
        paths_correspond_mod_chan pool convo c
          (path_r @ (DNxt n$_r$) # path_sfx)
          (static_path_pre @ (NBind n$_s$, ESend n$_s$e) # (NBind n$_r$, MNxt) # static_path_sfx))
\end{lstlisting}
...
\begin{lstlisting}[language=logic, mathescape]
  ...
  lemma not_static_traceable_sound: 
    \forall e0 pool convo path n b p' env stack static_env static_convo
      ...
      then
        (exists static_path . 
          paths_correspond_mod_chan pool convo (Chan path_c n$_c$) path static_path, 
          static_traceable graph entr exit (NBind n$_c$) is_end static_path)
  ...
  lemma not_static_inclusive_sound:
    \forall e0 pool convo static_env entr exit n$_c$ graph static_convo
      ...
      if
        ...
        static_traceable graph entr exit
          (NBind n$_c$) (static_send_id static_env e0 n$_c$) static_path1, 
        pool path2 = Some stt2, 
        paths_correspond_mod_chan pool convo (Chan path_c n$_c$) path2 static_path2, 
        static_traceable graph entr exit
          (NBind n$_c$) (static_send_id static_env e0 n$_c$) static_path2
      then
        static_inclusive static_path1 static_path2
  \end{lstlisting}
...
a framework for studying concurrency abstractions, synchronization mechanisms, and their
...
use clearer abstractions or allow parallelism for faster execution. On the other hand,
