...
(∀ .
    ⟦ 
      leaf trpl pi ;
      trpl pi = Some (⟨(Let x b e); env; k⟩) ;
      seq_step b env v
    ⟧ ⟹
    (trpl, ys) → (trpl(pi @ [LNxt x] ↦ ⟨e; env(x ↦ v); k⟩), ys)
(∀ .
    ⟦ 
      leaf trpl pi ;
      trpl pi = Some (⟨(Let x b e); env; k⟩) ;
    ...
    ⟧ ⟹
    (trpl, ys) → (trpl(pi @ [LCall x] ↦ ⟨e'; env'; (Ctn x e env) # k⟩), ys)
  " |
  Let_Chan: "
(∀ .
    ⟦ 
      leaf trpl pi ;
      trpl pi = Some (⟨(Let x MkChn e); env; k⟩)
    ⟧ ⟹
    (trpl, ys) → (trpl(
      pi @ [LNxt x] ↦ (⟨e; env(x ↦ (VChn (Ch pi x))); k⟩)), ys)
  " |
  Let_Spawn: "
(∀ .
    ⟦ 
      leaf trpl pi ;
      trpl pi = Some (⟨Let x (Spwn ec) e; env; k⟩)
    ⟧ ⟹
    (trpl, ys) → (trpl(
      pi @ [LNxt x] ↦ (⟨e; env(x ↦ VUnt); k⟩), 
      pi @ [LSpwn x] ↦ (⟨ec; env; []⟩)), ys)
  " |
  Let_Sync: "
(∀ .
    ⟦
      ...
      leaf trpl pis ;
      trpl pis = Some (⟨Let xs (Sync xse) es; envs; ks⟩);
      envs xse = Some (VClsr (SendEvt xsc xm) envse);

      leaf trpl pir ;
      trpl pir = Some (⟨Let xr (Sync xre) er; envr; kr⟩);
    ...
    (trpl, ys) → (
      trpl(
        pis @ [LNxt xs] ↦ (⟨es; envs(xs ↦ VUnt); ks⟩), 
        pir @ [LNxt xr] ↦ (⟨er; envr(xr ↦ vm); kr⟩)), 
      ys ∪ {(pis, c, pir)})
  "
