...
\begin{lstlisting}[language=sugar_lang, escapechar=\%]
...
\begin{lstlisting}[language=logic, escapechar=\%]
  ...
  predicate static_recv_site of
    ...
    (%$\forall$% prog0 x x_e e' x_rc env_a x_c .
      ...
      then
        static_recv_site env_a prog0 x_c (NLet x))
  \end{lstlisting}
...
liveness analysis handles recursion that does not contain any channel of interest.  No channel is
considered as live in the body of the loop.  However, a channel may be live before the loop is
...
the loops may not be live according to the entry map.
...
\includegraphics[width=1.0\textwidth, left]{cml_graph_lp.pdf}
\includegraphics[width=1.0\textwidth, left]{cml_graph_k1.pdf}
\includegraphics[width=1.0\textwidth, left]{cml_graph_k4.pdf}
