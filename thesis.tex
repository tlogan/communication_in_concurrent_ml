...
values. However, it's not absolutely necessary. Approximations of the dynamic values are
good enough.

static values are aproximations that consist of the static unit value, static channels, and
static primitive values.
The static unit value is actually no less precise than the dynamic unit value.
The static channel
is identified only by the name at which it is created, rather than the full control path that
leads up to it.  The static primitive values are simply the primitive syntax without an
