...
It could also be implemented as features of
a language within a compiler and runtime or interpreter.
...
any one of its latest state.  If there are threads
trying to send on the channel, the receiving 
operation dequeues an item from the state's associated queue.  The item consists of a condition
...
the enqueued condition.  Once a sending thread signals on the same condition, the receiving 
...
Control paths are simply sequences of let bound names.
...
\begin{lstlisting}[language=logic, mathescape]
  predicate dynamic_eval of
    ...
    (\forall pool path n p' env stack .
      if 
        leaf pool path,
        pool path = Some (Stt (Let n MkChn p') env stack)
      then
        dynamic_eval
          pool convo 
          (pool(path @ [DNxt n] ->
    ...
    (\forall pool path n p_c p' env stack convo .
      if 
        leaf pool path, 
        pool path = Some (Stt (Let n (Spwn p_c) p') env stack)
      then
        dynamic_eval
        pool convo 
        (pool(
    ...
    (\forall pool path_s path n$_s$ n$_s$e p_s env_s stack_s n$_s$c n_m env_se
      ...
      if 
        ...
        pool path_s = Some (Stt
          (Let n$_s$ (Sync n$_s$e) p_s) env_s stack_s),
        env_s n$_s$e = Some (VPrm
          (SendEvt n$_s$c n_m) env_se),
        leaf pool path_r,
        pool path_r = Some (Stt
          (Let n$_r$ (Sync n$_r$e) p$_r$) env_r stack_r),
...
\begin{lstlisting}[language=logic, mathescape]
  predicate static_reachable of program -> program -> bool:
    ...
    (\forall p_c p_z n p' . 
      ...
      then
        static_reachable (Let n (Spwn p_c) p') p_z),

    (\forall p$_l$ p_z n n$_s$ n$_l$ n$_r$ p$_r$ p' . 
      if
        static_reachable p$_l$ p_z
      then
        static_reachable (Let n (Case n$_s$ n$_l$ p$_l$ n$_r$ p$_r$) p') p_z),
    ...
    (\forall p$_b$ p_z n n$_f$ n$_p$ p$_b$ p' . 
      ...
      then
        static_reachable (Let n (Prim (Abs n$_f$ n$_p$ p$_b$)) p') p_z),

    (\forall p' p_z n b . 
      if
        static_reachable p' p_z
      then
        static_reachable (Let n b p') p_z)
  \end{lstlisting}
      ...
      if
        ...
        (\forall n$_s$c n_m n$_c$ y.
          if
            ...
            (SChn n$_c$) \in static_env n$_s$c,
            static_recv_program_id static_env p' n$_c$ (NLet y)
          then
            (NLet x, ESend n$_s$e, NLet y) \in F),
        static_traversable static_env graph p'
      then
        static_traversable static_env graph (Let n (Sync n$_s$e) p')),
  ...
  predicate paths_correspond_mod_chan of 
    ...
    (\forall pool path_r n$_r$ path_sfx stt  path_s n$_s$ n$_s$e p_sy env_sy stack_sy
      ...
      if
        ...
        pool path_s = Some (Stt (Let n$_s$ (Sync n$_s$e) p_sy) env_sy stack_sy),
        pool path_r = Some (Stt (Let n$_r$ (Sync n$_r$e) p$_r$y) env_ry stack_ry),
      ...
      then
        paths_correspond_mod_chan pool convo c
          ...
          (static_path_pre @ (NLet n$_s$, ESend n$_s$e) # (NLet n$_r$, MNxt) # static_path_sfx))
\end{lstlisting}
    ...
    \forall e0 pool convo path n b p' env stack static_env static_convo
      ...
      then
        (exists static_path . 
          ...
          static_traceable graph entr exit (NLet n$_c$) is_end static_path)
...
analyses can be built on the existing semantics, in order to verify the correctness of feature
...
next step.  One might also want to extend the language with syntax the identify blocks of code
that should be deterministic, along with a corresponding analysis that checks if such code is
deterministic \cite[]. Other analyses could aid optimizations for incremental computation
\cite[], by transforming a program into one that detects dynamically altered dependencies and
only recomputes those data with altered dependencies.
