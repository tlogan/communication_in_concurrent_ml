...
or without the manipulation explained. Some people like to restate the goal and the manipulation that leads
to the goal at the end of the proof, while others finish their proofs the intermediate theorem that
is easily manipulated to reach the goal. 

For instance, say you want to prove \lstinline[language=logic]{P1 or P2 -> Q}.  You might begin by 
stating the axiom \lstinline{P1 or P2 |- P1 or P2}, written as "assume P1 or P2 ...".
Perhaps you know theorem A: \lstinelin{|- P1 -> Q}. Using modus ponens, you derive {P1 |- Q}. Let's say
you also know theorem B: \lstinline{|- P2 -> Q}. Using modus ponens with theorem B, you derive {P2 |- Q}.
Your realize that these two theorems may be combined into {P1 or P2 |- Q}, which reduces to {|- P1 or P2 -> Q}.
...
assume P1 or P2
  case P1:
    have P1 -> Q by theorem A
    have Q by modus ponens
  have P1 |- Q 
  case P2:
    have P2 -> Q by theorem B    
    have Q by modus ponens
  have P2 |- Q
  Q by disjunction elimination
P1 or P2 |- Q
have P1 or P2 -> Q by implication introduction  
...
For proving simple propositions, it may be easy to conjure up the theorems or axioms needed to
combine and manipulate into the goal. However, for proving complex or unfamiliar propositions,
it may be less clear. In addition to the forward and declarative proof style of informal proofs, Isabelle/HOL
also supports an alternative way to structure proofs, by starting with the goal and breaking it into simpler
and simpler subgoals until axioms. The backwards style of reasoning, allows the proof
    ...
    fun send (Chn (ctntRef, lock)) m = 
      ...
      (case !ctntRef of
        ...
        let
          ...
          val () = msgCell := Some m
          val () = if (isEmpty q) then ctntRef := Inac else ()
        in
    ...
    fun recv (Chn (ctntRef, lock)) =  
      ...
      | Inac =>
        let
          val recvCond = condition ()
          val msgCell = ref NONE 
          val () = ctntRef := Recv (queue [(recvCond, msgCell)])
          val () = release lock; wait recvCond
        in
          valOf (!msgCell)
        end
...
\begin{lstlisting}[language=logic, mathescape]
  ...
  * \forall n$_s$ n$_l$ t$_l$ n$_r$ t$_r$ .
      freeVarsComplex (Case n$_s$ n$_l$ t$_l$ n$_r$ t$_r$) = 
        {n$_s$} \union freeVarsTerm t$_l$ \union freeVarsTerm t$_r$ \ {n$_l$, n$_r$}
  * \forall n$_f$ n$_a$ .
  ...
  where only
    \forall n c t .
      freeVarsTerm (Bind n c t) = freeVarsComplex c \union freeVarsTerm t \ {n}
  * \forall n .
