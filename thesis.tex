...
\begin{lstlisting}[language=logic, mathescape]
  ...
  fun top_term_id of term -> term_id:
  (\forall n c t' . 
    top_term_id (Bind n c t') = NBnd x
  ),
  (\forall n . 
    top_term_id (Rslt n) = NRslt x
  )
...
The static acceptance for flows describes a set of flows consisting of all the flows
...
\begin{lstlisting}[language=logic, mathescape]
  ...
  (\forall n t' graph static_env  .
    if
      (NBnd n , MNxt, top_term_id t') \in graph,
  ...
  (\forall n t' p_c graph static_env.
    if
      {(NBnd x, MNxt, top_term_id t'),
        (NBnd x, MSpwn, top_term_id p_c)} \subseteq graph, 
  ...
  (\forall n t$_l$ t$_r$ t' graph static_env n$_s$ .
    if
      {
        (NBnd x, MCll, top_term_id t$_l$),
        (NBnd x, MCll, top_term_id t$_r$),
        (NResult (result_name t$_l$), MRtn, top_term_id t'),
        (NResult (result_name t$_r$), MRtn, top_term_id t')
      } \subseteq graph, 
  ...
  (\forall  static_env n$_f$ n t' n n$_a$ .
    if
      (\forall n$_f$' n$_t$ t$_b$ . if (SAtm (Fun n$_f$' n$_t$ t$_b$)) \in static_env n$_f$ then 
        {
          (NBnd x, MCll, top_term_id t$_b$),
          (NResult (result_name t$_b$), MRtn, top_term_id t')
        } \subseteq graph
      ),
      static_flows_accept static_env graph t'
    then
...
The server implementation represented as a graph illustrates how static acceptance for flows can interpret
...
\begin{lstlisting}[language=logic, mathescape]
  predicate static_traceable of
    flow set -> term_id -> (term_id -> bool) -> static_path -> bool:
    only
...
\begin{lstlisting}[language=logic, mathescape]
  predicate static_one_shot of static_value_map -> term -> name -> bool:
    ...
    (\forall graph p static_env n$_c$ .
      if
        every_two
          (static_traceable graph (top_term_id e) (static_send_id static_env p n$_c$))
          singular,
    ...
    (\forall graph p static_env n$_c$ .
      if
        (every_two
          ...
          noncompetitive), 
        (every_two
          (static_traceable graph (top_term_id e) (static_recv_term_id static_env p n$_c$))
          noncompetitive), 
        static_flows_accept static_env graph e
      then
    ...
    \forall e0 pool comm path n b p' env stack evn_a static_comm graph is_end .
      ...
      then
        exists static_path . 
          ...
          static_traceable graph (top_term_id e0) is_end static_path
...
static acceptance for flows and static evaluation over pools, such that information about a step in
...
The generalization of static acceptance for flows is composed of in static accepting over values,
static acceptance for flows over environments, static acceptance for flows over stacks, and static
...
static acceptance for flows over a non-empty stack, where
...
traversability over programs, but provides direct information about a flow in the
graph, which would otherwise only be deducible by a varying number of logical steps
depending on the program.

\begin{lstlisting}[language=logic, mathescape]
  predicate static_flows_accept_val of
    ...
    (\forall static_env graph c .
      static_flows_accept_val static_env graph (VChn c)),
  ...
  predicate static_flows_accept_env of
  ...
  predicate static_flows_accept_stack of
    ...
    (\forall y p graph static_env graph env stack n env .
      if 
        {(NResult y, MRtn, top_term_id e)} \subseteq graph,
        static_flows_accept static_env graph e,
        static_flows_accept_env static_env graph env,
...
The flows described by the various versions of static acceptance for flows depend on static
...
As with the generalized versions of static acceptance for flows,
  ...
  lemma static_flows_accept_pool_preserved_star:
    \forall e0 pool comm static_env static_comm graph .
      if
        star dynamic_eval [[] -> (Stt e0 [->] [])] {} pool comm, 
        static_eval static_env static_comm e0,
        static_flows_accept_pool static_env graph [[] -> (Stt e0 [->] [])]
      then
        static_flows_accept_pool static_env graph pool 
\end{lstlisting}
...
of static traceability. The preservation of static acceptance for flows over pools is proved by the
...
The static acceptance for flows is similar to that of the previous analysis.
...
\begin{lstlisting}[language=logic, mathescape]
  predicate static_flows_accept of static_value_map -> graph -> term -> bool:
    ...
    (\forall n p' graph static_env  .
      if
        (NBnd n , MNxt, top_term_id p') \in graph,
    ...
    (\forall n p' graph static_env n$_s$e .
      if
        ...
        (\forall n$_sc$ n$_m$ n$_c$ y.
          ...
          then
            (NBnd x, ESend n$_s$e, NBnd y) \in F),
        static_flows_accept static_env graph p'
      then
    ...
    (\forall n p$_l$ p$_r$ p' graph static_env n$_s$ .
      if
        {(NBnd x, MCll, top_term_id p$_l$),
          (NBnd x, MCll, top_term_id p$_r$),
          (NResult (result_name p$_l$), MRtn, top_term_id p'),
          (NResult (result_name p$_r$), MRtn, top_term_id p')} \subseteq graph,
    ...
    (\forall  static_env n$_f$ n p' n n$_a$ .
      if
        (\forall n$_f$' n$_p$ p$_b$ . if (SAtm (Fun n$_f$' n$_p$ p$_b$)) \in static_env n$_f$ then 
          {(NBnd x, MCll, top_term_id p$_b$),
            (NResult (result_name p$_b$), MRtn, top_term_id p')} \subseteq graph),
  ...
  predicate static_live_chan of
    ...
    (\forall exit n entr p' static_env n$_c$ .
      if 
        ...
        entr (top_term_id p') \subseteq exit (NBnd x),
    ...
    (\forall exit n entr static_env n$_c$ n$_a$ p' . 
      if
        ...
        (if static_built_on_chan static_env entr n$_c$ n$_a$ then
          {x_a} \subseteq entr (NBnd x))
        entr (top_term_id e) \subseteq exit (NBnd x),
    ...
    (\forall  exit n entr p$_b$ n$_p$ n static_env n$_c$ p' n$_f$ . 
      if
        (exit (NBnd x) - {x}) \subseteq entr (NBnd x),
        (entr (top_term_id p$_b$) - {x_p}) \subseteq entr (NBnd x),
    ...
    (\forall exit n entr p' p_c n$_c$ static_env .
      if
        (exit (NBnd x) - {x}) \subseteq entr (NBnd x),
        entr (top_term_id p') \subseteq exit (NBnd x),
        entr (top_term_id p_c) \subseteq exit (NBnd x),
    ...
    (\forall exit n entr p$_l$ n$_l$ p$_r$ n$_r$ static_env n$_c$ n$_s$ p' .
      if 
        (exit (NBnd x) - {x}) \subseteq entr (NBnd x),
        (entr (top_term_id p$_l$) - {x_l}) \subseteq entr (NBnd x),
        (entr (top_term_id p$_r$) - {x_r}) \subseteq entr (NBnd x),
