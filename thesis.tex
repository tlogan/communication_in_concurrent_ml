...
After the receiving thread signal on \lstine{sendCond}, the sending operation returns the unit value.
...
such as \lsininline{replCh} in the server example.
...
all possible executions of execution of a program. However, some programs run forever, so it may never be possible to
