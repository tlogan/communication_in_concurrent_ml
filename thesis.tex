...
any one of its latest state.  If there are threads trying to send on the channel, the receiving 
operation dequeues an item from the state's associated queue.  The item consists of a condition
...
Instead of relying on additional meta-syntax for program points of primitive operations, the analysis 
can simply use the required names of the restricted grammar to identify locations in the program.
Control paths are simply sequences of let bound names. A modification of the example Server
implementation in ML illustrates the restrictive grammar applied to Concurrent ML. In order
to preserve space and readability, the grammar used
in this example is not quite as strict as one that will be used the for
...
\begin{lstlisting}[language=logic, escapechar=\%]
  predicate static_traversable of
    static_environment -> static_program -> program -> bool:
    only
...
\begin{lstlisting}[language=logic, escapechar=\%]
  predicate static_traversable_val of
    static_environment -> static_program -> dynamic_value -> bool:
    only
    (%$\forall$% env_a prog_a .
  ...
  predicate static_traversable_env of
    static_environment -> static_program -> env -> bool: 
    only 
    (%$\forall$% env_a prog_a env .
      if
  ...
  predicate static_traversable_stack of
    static_environment -> static_program ->
  ...
  predicate static_traversable_pool of
    static_environment -> static_program -> pool -> bool:
    only
...
\begin{lstlisting}[language=logic, escapechar=\%]
  predicate static_traversable of static_environment -> static_program -> program -> bool:
    only
    (%$\forall$% env_a prog_a x .
...
\begin{lstlisting}[language=logic, escapechar=\%]
  predicate static_traceable of
    static_environment -> static_program -> program_id_map -> program_id_map ->
