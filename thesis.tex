...
predicate seq_step of bound_exp -> env -> val -> bool:
only
(∀ env . 
...
predicate seq_step_up of bound_exp -> env -> exp -> env -> bool:
only
...
predicate leaf of trace_pool -> control_path -> bool
(∀ pool path stt .
  if
...
predicate is_send_path of trace_pool -> chan -> control_path -> bool:
only
...
predicate is_recv_path of trace_pool -> chan -> control_path -> bool:
only
...
predicate one_shot of trace_pool -> chan -> bool:
(∀ pool c .
  if
...
predicate fan_out of trace_pool -> chan -> bool:
(∀ pool c .
  if
...
predicate fan_in of trace_pool -> chan -> bool:
only
(∀ pool c .
  if
...
predicate one_to_one of trace_pool -> chan -> bool:
only
(∀ pool c.
  if
    fan_out pool c and 
...
predicate static_eval of abstract_env -> abstract_env -> exp -> bool:
only
...
predicate static_reachable of exp -> exp -> bool:
only
(∀ e .
...
predicate static_send_label of abstract_env -> exp -> var -> label -> bool:
only
...
predicate static_recv_label of abstract_env -> exp -> var -> label -> bool:
only
...
predicate static_traversable of abstract_env -> transition_set -> exp -> bool:
only
(∀ env_a graph x .
...
predicate static_one_to_one of abstract_env -> exp -> var -> bool:
only
(∀ graph e env_a x_c .
  if
    (every_two
...
predicate static_fan_out of abstract_env -> exp -> var -> bool:
only
(∀ graph e env_a x_c .
  if
    (every_two
...
predicate static_traversable_val of abstract_env -> transition_set -> val -> bool:
...
(∀ env_a graph e_b env f x_p .
  if
    ...
    static_traversable_env env_a graph env
  then
    static_traversable_val env_a F (VClsr (Abs f x_p e_b) env)) and
...
fun abstract of val -> abstract_value:
...
The predicate static_reachable follows
...
predicate static_reachable_over_prim of exp -> prim -> bool:
only
(∀ e0 x_c x_m .
...
predicate static_reachable_val of exp -> val -> bool:
only
(∀ e0 .
...
predicate static_reachable_env of exp -> env -> bool:
only
(∀ e0 env
  if
...
predicate static_reachable_over_stack of exp -> contin list -> bool:
only
(∀ e0 .
...
predicate static_reachable_pool of exp -> trace_pool -> bool:
only
(∀ e0 pool .
  then
    (∀ path e env stack . if pool path = Some (Stt e env stack) then 
...
the sending mode, as indicated by the new definition of static_traversable.  Modes for typical
...
static_built_on_chan states that a variable is bound to some structure containing a channel. 
...
predicate static_built_on_chan of
  abstract_env -> label_map -> var -> var -> bool:
...
(∀ x_sc x_m env_a x entr x_c . 
  if
    (AClsr (SendEvt x_sc x_m)) ∈ env_a x and
...
(∀ x_rc env_a x entr x_c . 
  if  
    (AClsr (RecvEvt x_rc)) ∈ env_a x and
...
Using the predicate static_built_on_chan, the predicate static_live_chan completes the
...
predicate static_live_chan of abstract_env -> label_map -> label_map -> var -> exp -> bool:
...
(∀  exit x entr e_b x_p x env_a x_c e' f . 
  if
    (exit (NLet x) - {x}) ∪ (entr (top_label e_b) - {x_p}) ⊆ entr (NLet x) and
...
(∀ exit x entr e' e_c x_c env_a .
  if
    (exit (NLet x) - {x}) ⊆ entr (NLet x) and
    entr (top_label e') ∪ entr (top_label e_c) ⊆ exit (NLet x) and
...
The predicate static_live_traversable states that an abstraction transition exists in some
graph, and is considered live with respect to given entry and exit functions.  The predicate
static_live_traceable states that an entire abstract path exists in some graph and is live with
...
(∀ l l' graph entr exit .
  if
    (l, EReturn, l') ∈ graph and
    not (entr l' = {})
  then
    static_live_traversable graph entr exit (l, EReturn, l'))

(∀ x_send x_evt x_recv graph entr exit .
  if
    ((NLet x_send), ESend x_evt, (NLet x_recv)) ∈ graph and 
...
(∀ is_end start env_a graph entr exit .
  if
    is_end start
  then
...
(∀ graph entr exit start middle path is_end mode. 
  if
    static_live_traceable graph entr exit start (λ l . l = middle) path and 
    (is_end end) and
...
static_inclusive describes relationship between paths for those paths to possible occur within
...
The predicates static_one_shot, static_one_to_one, static_fan_out, and static_fan_in describe
...
predicate static_one_shot of abstract_env -> exp -> var -> bool:

only

(∀ graph entr exit x_c env_a e . 
  if
    every_two
...
predicate static_fan_in of abstract_env -> exp -> var -> bool:
...
analysis.  These reasoning strategies are demonstrated by the theorem static_one_shot_sound. 
...
theorem static_one_shot_sound: "
  ⟦
    ...
    (V, C) ⊨⇩e e;
    star concur_step ([[] ↦ ⟨e;Map.empty;[]⟩], {}) (ℰ', H')
  ⟧ ⟹
  one_shot ℰ' (Ch π xC)
"


The theorem static_one_shot_sound depends on correlating dynamic paths with abstract paths,
which is described by the predicates paths_correspond and paths_correspond_mod_chan.
Additionally the theorem follows from not_static_traceable_sound,
not_static_inclusive_sound and not_send_site_sound.  The reasoning for the latter one, is
identical to that of the lower precision analysis, but the the reasoning for the former two is
significantly more complicated and not yet completed.



predicate paths_correspond of control_path -> abstract_path -> bool:
only
...
(∀ .
  ...
  then
    paths_correspond (π @ [LNxt x]) (path @ [(NLet x, ENext)])) and

(∀ .
  if
    paths_correspond π path
  then
    paths_correspond (π @ [LSpwn x]) (path @ [(NLet x, ESpawn)])) and

(∀ .
  if
    paths_correspond π path
  then
    paths_correspond (π @ [LCall x]) (path @ [(NLet x, ECall)])) and

(∀ .
  if
    paths_correspond π path
  then
    paths_correspond (π @ [LRtn y]) (path @ [(NResult y, EReturn)]))


predicate paths_correspond_mod_chan of 
  trace_pool -> cmmn_set -> chan -> control_path -> abstract_path -> bool:

only

(∀ .
  if
    paths_correspond ((LNxt xC) # πSuff) path ⟹
    ℰ (πC @ (LNxt xC) # πSuff) ≠ None
  then
    paths_correspond_mod_chan (ℰ, H) (Ch πC xC) (πC @ (LNxt xC) # πSuff) path) and

(∀ . 
  if
    paths_correspond πSuffix pathSuffix and 
    ℰ (πR @ (LNxt xR) # πSuffix) ≠ None and 
    dynamic_built_on_chan_var ρRY c xR and 
    ℰ πS = Some (⟨(Let xS (Sync xSE) eSY);ρSY;κSY⟩) and
    ℰ πR = Some (⟨(Let xR (Sync xRE) eRY);ρRY;κRY⟩) and
    {(πS, c, πR)} ⊆ H and 
    paths_correspond_mod_chan (ℰ, H) c πS pathPre
  then
    paths_correspond_mod_chan ℰ H c
      (πR @ (LNxt xR) # πSuffix)
      (pathPre @ (NLet xS, ESend xSE) # (NLet xR, ENext) # pathSuffix))



lemma not_static_traceable_sound: 
∀ e0 pool convo path x b e' env stack env_a convo_a entr exit x_c graph is_end path_c  . 
  if
    star concur_step [[] -> (Stt e0 [->] []] {} pool convo and
    pool path = Some (Stt (Let x b e') env stack) and
    static_eval env_a convo_a e0 and
...
∀ .
  if
    star concur_step ([[] ↦ ⟨e;Map.empty;[]⟩], {}) (ℰ', H') and 
    static_live_chan V Ln Lx xC e and 
    static_traversable V F e and 
    (V, C) ⊨⇩e e and 
    ℰ' π1 ≠ None and 
    paths_correspond_mod_chan (ℰ', H') (Ch π xC) π1 path1 and 
    static_live_traceable F Ln Lx (NLet xC) (static_send_label V e xC) path1 and 
    ℰ' π2 ≠ None and 
    paths_correspond_mod_chan (ℰ', H') (Ch π xC) π2 path2 and 
    static_live_traceable F Ln Lx (NLet xC) (static_send_label V e xC) path2
  then
...
\begin{lstlisting}[style=codestyle1, escapechar=\%]
  ...
  inductive
    ...
    let_unt :
      {AUnt} %$\subseteq$% env_a x %$\Longrightarrow$%
      static_eval_exp (V, C) e %$\Longrightarrow$% 
      static_eval_exp (V, C) (Let x Unt e) |
    let_chan : 
      {AChn x} %$\subseteq$% env_a x  %$\Longrightarrow$%
      static_eval_exp (V, C) e %$\Longrightarrow$%
      static_eval_exp (V, C) (Let x (MkChn) e) |
    let_send_evt : 
      {APrim (SendEvt xc xm)} %$\subseteq$% env_a x %$\Longrightarrow$%
      static_eval_exp (V, C) e %$\Longrightarrow$% 
      static_eval_exp (V, C)
        (Let x (Prim (SendEvt xc xm)) e) |
    let_recv_evt :
      {APrim (RecvEvt xc)} %$\subseteq$% env_a x %$\Longrightarrow$%
      static_eval_exp (V, C) e %$\Longrightarrow$% 
      static_eval_exp (V, C)
        (Let x (Prim (RecvEvt xc)) e) |
    let_pair : 
      {APrim (Pair x1 x2)} %$\subseteq$% env_a x %$\Longrightarrow$%
      static_eval_exp (V, C) e %$\Longrightarrow$% 
      static_eval_exp (V, C) (Let x (Pair x1 x2) e) |
    let_left : 
      {APrim (Left xp)} %$\subseteq$% env_a x %$\Longrightarrow$%
      static_eval_exp (V, C) e %$\Longrightarrow$% 
      static_eval_exp (V, C) (Let x (Left xp) e) |
    let_right :
      {APrim (Right xp)} %$\subseteq$% env_a x %$\Longrightarrow$%
      static_eval_exp (V, C) e %$\Longrightarrow$% 
      static_eval_exp (V, C) (Let x (Right xp) e) |
    let_abs : 
      {APrim (Abs f' x' e')} %$\subseteq$% env_a f' %$\Longrightarrow$%
      static_eval_exp (V, C) e' %$\Longrightarrow$%
      {APrim (Abs f' x' e')} %$\subseteq$% env_a x %$\Longrightarrow$%
      static_eval_exp (V, C) e %$\Longrightarrow$% 
    ...
    let_sync  : 
      %$\forall$% xsc xm xc . 
        (APrim (SendEvt xsc xm)) %$\in$% env_a xe %$\longrightarrow$% 
        AChn xc %$\in$% env_a xsc %$\longrightarrow$%
        {AUnt} %$\subseteq$% env_a x %$\land$% env_a xm %$\subseteq$% C xc %$\Longrightarrow$%
      %$\forall$% xrc xc . 
        (APrim (RecvEvt xrc)) %$\in$% env_a xe %$\longrightarrow$%
        AChn xc %$\in$% env_a xrc %$\longrightarrow$%
        C xc %$\subseteq$% env_a x %$\Longrightarrow$%
      static_eval_exp (V, C) e %$\Longrightarrow$%  
      static_eval_exp (V, C) (Let x (Syync xe) e) |
    let_fst : 
      %$\forall$% x1 x2.
        (APrim (Pair x1 x2)) %$\in$% env_a xp %$\longrightarrow$%
        env_a x1 %$\subseteq$% env_a x %$\Longrightarrow$% 
      static_eval_exp (V, C) e %$\Longrightarrow$% 
      static_eval_exp (V, C) (Let x (Fst xp) e) |
    let_snd : 
    %$\forall$% x1 x2 .
      (APrim (Pair x1 x2) %$\in$% env_a xp %$\longrightarrow$%
      env_a x2 %$\subseteq$% env_a x %$\Longrightarrow$% 
    static_eval_exp (V, C) e %$\Longrightarrow$% 
    static_eval_exp (V, C) (Let x (Snd xp) e) |
  let_case :
    %$\forall$% xl' .
      (APrim (Left xl')) %$\in$% env_a xs %$\longrightarrow$%
        env_a xl' %$\subseteq$% env_a xl %$\land$% env_a (rslt_var el) %$\subseteq$% env_a x %$\land$%
        static_eval_exp (V, C) el %$\Longrightarrow$%
    %$\forall$% xr' .
      (APrim (Right xr')) %$\in$% env_a xs %$\longrightarrow$%
        env_a xr' %$\subseteq$% env_a xr %$\land$% env_a (rslt_var er) %$\subseteq$% env_a x %$\land$%
        static_eval_exp (V, C) er %$\Longrightarrow$%
        static_eval_exp (V, C) e %$\Longrightarrow$% 
      static_eval_exp (V, C) (Let x (Case xs xl el xr er) e) |
  let_app :
    %$\forall$% f' x' e' .
      (APrim (Abs f' x' e') %$\in$% env_a f %$\longrightarrow$%
      env_a xa %$\subseteq$% env_a x' %$\land$%
      env_a (rslt_var e') %$\subseteq$% env_a x %$\Longrightarrow$% 
    static_eval_exp (V, C) e %$\Longrightarrow$% 
...
\begin{lstlisting}[style=codestyle1, escapechar=\%]
  ...
  inductive 
    ...
    intro :  
      %$\forall$% x v .
        env x = Some v %$\longrightarrow$%
        {abstract v} %$\subseteq$% env_a x %$\land$% static_eval_val (V, C) v %$\Longrightarrow$%
...
\begin{lstlisting}[style=codestyle1, escapechar=\%]
  ...
  inductive static_eval_stack ::
    ...
    Nonempty : 
      valset %$\subseteq$% env_a x %$\Longrightarrow$%
      static_eval_exp (V, C) e %$\Longrightarrow$%
      static_eval_env (V, C) env %$\Longrightarrow$%
...
locale communication_sound =
  ...
  assumes 
    static_one_shot_sound:
      static_one_shot env_a e xC %$\Longrightarrow$%
    ...
    static_fan_out_sound: 
      static_fan_out env_a e xC %$\Longrightarrow$%
      (V, C) static_eval e %$\Longrightarrow$%
      star concur_step ([[] %$\mapsto$% (Stt e (%$\lambda$% _ . None) [])], {}) (trpl', H') %$\Longrightarrow$%
      fan_out trpl' (Ch pi xC) and

    static_fan_in_sound:
      static_fan_in env_a e xC %$\Longrightarrow$%
      (V, C) static_eval e %$\Longrightarrow$%
      star concur_step ([[] %$\mapsto$% (Stt e (%$\lambda$% _ . None) [])], {}) (trpl', H') %$\Longrightarrow$%
      fan_in trpl' (Ch pi xC) and

    static_one_to_one_sound: "
      static_one_to_one env_a e xC %$\Longrightarrow$%
      (V, C) static_eval e %$\Longrightarrow$%
      star concur_step ([[] %$\mapsto$% (Stt e (%$\lambda$% _ . None) [])], {}) (trpl', H') %$\Longrightarrow$%
...
\begin{lstlisting}[style=codestyle1, escapechar=\%]
  ...
  inductive
    ...
    intro:
      {AChn xC} %$\subseteq$% env_a xSC %$\Longrightarrow$%
      {APrim (SendEvt xSC xM)} %$\subseteq$% env_a xE %$\Longrightarrow$%
      static_reachable e (Let x (Sync xE) e') %$\Longrightarrow$%
      static_static_send_label env_a e xC (NLet x)
  ...
  inductive
    static_static_recv_label ::
      ...
      var %$\Rightarrow$% label %$\Rightarrow$% bool where
    intro:
      {AChn xC} %$\subseteq$% env_a xRC %$\Longrightarrow$%
      {APrim (RecvEvt xRC)} %$\subseteq$% env_a xE %$\Longrightarrow$%
      static_reachable e (Let x (Sync xE) e') %$\Longrightarrow$%
      static_static_recv_label env_a e xC (NLet x)
...
\begin{lstlisting}[style=codestyle1, escapechar=\%]
  inductive
    ...
    let_App:
      %$\forall$% fp xp eb .
        APrim (Abs fp xp eb) %$\in$% env_a f %$\longrightarrow$%
        {
...
\begin{lstlisting}[style=codestyle1, escapechar=\%]
  ...
  Empty:
    ...
    static_traceable env_a F start is_end [] |
  Edge:
    is_end end %$\Longrightarrow$%
    {(start, edge, end)} %$\subseteq$% F %$\Longrightarrow$%
    static_traceable env_a F start is_end [(start, edge)] |
  Step:
    static_traceable env_a F middle is_end ((middle, edge') # path) %$\Longrightarrow$%
    {(start, edge, middle)} %$\subseteq$% F %$\Longrightarrow$%
    static_traceable env_a F start is_end ((start, edge) # (middle, edge') # path)
  \end{lstlisting}
...
\begin{lstlisting}[style=codestyle1, escapechar=\%]
  inductive
    ...
    Sync:
      every_two
        (static_traceable env_a F (top_label e)
          (static_static_send_label env_a e xC))
        singular %$\Longrightarrow$%
      static_traversable env_a graph e %$\Longrightarrow$%
      static_one_shot env_a e xC 
    ...
    inductive
      ...
      Sync:
        every_two
          (static_traceable env_a F
            (top_label e) (static_static_send_label env_a e xC))
          noncompetitive %$\Longrightarrow$%
        every_two
          (static_traceable env_a F
            (top_label e) (static_static_recv_label env_a e xC))
          noncompetitive %$\Longrightarrow$%
        static_traversable env_a graph e %$\Longrightarrow$%
        static_one_to_one env_a e xC 
  ...
  inductive
    static_fan_in ::
      ...
      var %$\Rightarrow$% bool where
    Sync:
      every_two (static_traceable env_a F (top_label e) (static_static_recv_label env_a e xC)) noncompetitive %$\Longrightarrow$%
      static_traversable env_a graph e %$\Longrightarrow$%
      static_fan_in env_a e xC
...
\begin{lstlisting}[style=codestyle1, escapechar=\%]
  inductive 
    ...
    Intro:
      %$\forall$% x v .
        env x = Some v %$\longrightarrow$%
        {rslt_var v} %$\subseteq$% env_a x %$\land$% static_traversable_val env_a F v %$\Longrightarrow$%
      static_traversable_env env_a F env |

    Unit:
      static_traversable_val env_a F VUnit |
    Chan:
      static_traversable_val env_a F (VChn c) |
    SendEvt:
      static_traversable_env env_a F env %$\Longrightarrow$%
      static_traversable_val env_a F (VClsr (SendEvt _ _) env) |
    RecvEvt:
      static_traversable_env env_a F env %$\Longrightarrow$%
      static_traversable_val env_a F (VClsr (RecvEvt _) env) |
    Left:
      static_traversable_env env_a F env %$\Longrightarrow$%
      static_traversable_val env_a F (VClsr (Left _) env) |
    Right:
      static_traversable_env env_a F env %$\Longrightarrow$%
      static_traversable_val env_a F (VClsr (Right _) env) |
    Abs:
      static_traversable env_a graph e %$\Longrightarrow$% 
      static_traversable_env env_a F  env %$\Longrightarrow$%
      static_traversable_val env_a F (VClsr (Abs f x e) env) |
  Pair:
    static_traversable_env env_a F env %$\Longrightarrow$%
    static_traversable_val env_a F (VClsr (Pair _ _) env) 
  ...
  inductive static_traversable_stack ::
    ...
    Empty:
      static_traversable_stack env_a F [] |
    Nonempty:
      static_traversable env_a graph e %$\Longrightarrow$%
      static_traversable_env env_a F env %$\Longrightarrow$%
      static_traversable_stack env_a F k %$\Longrightarrow$% 
      static_traversable_stack env_a F ((Ctn x e env) # k))
  ...
  inductive
    ...
    Intro:
      %$\forall$% pi e env k .
        ...
        static_traversable_env env_a F env %$\land$%
        static_traversable_stack env_a F k %$\Longrightarrow$% 
      static_traversable_pool env_a F E
...
\begin{lstlisting}[style=codestyle1, escapechar=\%]
  ...
  lemma static_traversable_pool_preserved_star: "
    static_traversable_pool env_a F
    ...
    static_traversable_pool env_a F trpl' "
  proof sketch
  qed
...
lemma static_traversable_pool_implies_static_traceable: "
  ...
  concur_step
    ...
    (trpl', ys') %$\Longrightarrow$% 
  static_eval (V, C) e %$\Longrightarrow$%
  static_traversable_pool env_a F trpl' %$\Longrightarrow$%
  is_end (NLet x) %$\Longrightarrow$%
  %$\exists$% path . 
    paths_correspond pi path %$\land$%
    static_traceable env_a F (top_label e) is_end path "
  proof sketch
  qed
  \end{lstlisting}
...
\begin{lstlisting}[style=codestyle1, escapechar=\%]
  ...
  let_Sync:
    {(NLet x, ENxt, top_label e)} %$\subseteq$% F %$\Longrightarrow$%
    (%$\forall$% xSC xM xC y .
      {^SendEvt xSC xM} %$\subseteq$% env_a xSE %$\longrightarrow$%
      {AChan xC} %$\subseteq$% env_a xSC %$\longrightarrow$%
  ...
  let_App:
      (%$\forall$% f' xp eb . ^Abs f' xp eb %$\in$% env_a f %$\longrightarrow$%
        {(NLet x, ECall, top_label eb),
...
where
  Chan:
      AChn xc %$\in$% env_a x %$\Longrightarrow$% 
      static_built_on_chan env_a entr xc x |
  SendEvt: 
      APrim (SendEvt xsc xm) %$\in$% env_a x %$\Longrightarrow$% 
      static_built_on_chan env_a entr xc xsc %$\lor$% static_built_on_chan env_a entr xc xm %$\Longrightarrow$% 
      static_built_on_chan env_a entr xc x |
  RecvEvt:
    APrim (RecvEvt xrc) %$\in$% env_a x %$\Longrightarrow$% 
    static_built_on_chan env_a entr xc xrc %$\Longrightarrow$% 
    static_built_on_chan env_a entr xc x |
  Pair:
      APrim (Pair x1 x2) %$\in$% env_a x %$\Longrightarrow$%
      static_built_on_chan env_a entr xc x1 %$\lor$% static_built_on_chan env_a entr xc x2 %$\Longrightarrow$% 
      static_built_on_chan env_a entr xc x |
  Left:
      APrim (Left xa) %$\in$% env_a x %$\Longrightarrow$% 
      static_built_on_chan env_a entr xc xa %$\Longrightarrow$% 
    static_built_on_chan env_a entr xc x |
  Right:
      APrim (Right xa) %$\in$% env_a x %$\Longrightarrow$% 
      static_built_on_chan env_a entr xc xa %$\Longrightarrow$% 
      static_built_on_chan env_a entr xc x |
  Abs:
    APrim (Abs f xp eb) %$\in$% env_a x %$\Longrightarrow$% 
    %$\not$% Set.is_empty (entr (labelLabel eb) - {xp}) %$\Longrightarrow$%
    static_built_on_chan env_a entr xc x 
  \end{lstlisting}
...
\begin{lstlisting}[style=codestyle1, escapechar=\%]
  fun chan_set ::
    ...
    chan_set env_a entr xc x = (if (static_built_on_chan env_a entr xc x) then {x} else {})
  \end{lstlisting}
...
\begin{lstlisting}[style=codestyle1, escapechar=\%]
  inductive static_live_chan ::
    ...
    Result:
      chan_set env_a entr xc y = entr (NRslt y) %$\Longrightarrow$%
      static_live_chan env_a entr exit xc (Rslt y) |
  Let_Unit:
      static_live_chan env_a entr exit xc e %$\Longrightarrow$%
      entr (top_label e) = exit (NLet x) %$\Longrightarrow$%
      exit (NLet x) = entr (NLet x) %$\Longrightarrow$%
      static_live_chan env_a entr exit xc (Let x Unt e) |
  Let_Chan:
    static_live_chan env_a entr exit xc e %$\Longrightarrow$%
    entr (top_label e) = exit (NLet x) %$\Longrightarrow$%
    (exit (NLet x) - {x}) = entr (NLet x) %$\Longrightarrow$%
    static_live_chan env_a entr exit xc (Let x MkChn e) |
  Let_SendEvt:
    static_live_chan env_a entr exit xc e %$\Longrightarrow$%
    entr (top_label e) = exit (NLet x) %$\Longrightarrow$%
    (exit (NLet x) - {x}) %$\cup$% chan_set env_a entr xc xsc %$\cup$%
      chan_set env_a entr xc xm = entr (NLet x) %$\Longrightarrow$%
    static_live_chan env_a entr exit xc (Let x (Prim (SendEvt xsc xm)) e) |
  Let_RecvEvt:
    static_live_chan env_a entr exit xc e %$\Longrightarrow$%
    entr (top_label e) = exit (NLet x) %$\Longrightarrow$%
    (exit (NLet x) - {x}) %$\cup$% chan_set env_a entr xc xrc = entr (NLet x) %$\Longrightarrow$%
    static_live_chan env_a entr exit xc (Let x (Prim (RecvEvt xrc)) e) |
  Let_Pair:
      static_live_chan env_a entr exit xc e %$\Longrightarrow$%
      entr (top_label e) = exit (NLet x) %$\Longrightarrow$%
      (exit (NLet x) - {x}) %$\cup$%  chan_set env_a entr xc x1 %$\cup$%
        chan_set env_a entr xc x2 = entr (NLet x) %$\Longrightarrow$%
      static_live_chan env_a entr exit xc (Let x (Prim (Pair x1 x2)) e) |
  Let_Left:
      static_live_chan env_a entr exit xc e %$\Longrightarrow$%
      entr (top_label e) = exit (NLet x) %$\Longrightarrow$%
      (exit (NLet x) - {x}) %$\cup$% chan_set env_a entr xc xa = entr (NLet x) %$\Longrightarrow$%
      static_live_chan env_a entr exit xc (Let x (Prim (Lft xa)) e) |
  Let_Right:
      static_live_chan env_a entr exit xc e %$\Longrightarrow$%
      entr (top_label e) = exit (NLet x) %$\Longrightarrow$%
      (exit (NLet x) - {x}) %$\cup$% chan_set env_a entr xc xa = entr (NLet x) %$\Longrightarrow$%
      static_live_chan env_a entr exit xc (Let x (Prim (Rght xa)) e) |
  Let_Abs:
      static_live_chan env_a entr exit xc e %$\Longrightarrow$%
      entr (top_label e) = exit (NLet x) %$\Longrightarrow$%
      static_live_chan env_a entr exit xc eb %$\Longrightarrow$%
      (exit (NLet x) - {x}) %$\cup$%
        (entr (top_label eb) - {xp}) = entr (NLet x) %$\Longrightarrow$%
      static_live_chan env_a entr exit xc (Let x (Prim (Abs f xp eb)) e) |
  Let_Spawn:
      static_live_chan env_a entr exit xc e %$\Longrightarrow$%
      static_live_chan env_a entr exit xc ec %$\Longrightarrow$%
      entr (top_label e) %$\cup$% entr (top_label ec) = exit (NLet x) %$\Longrightarrow$%
      (exit (NLet x) - {x}) = entr (NLet x) %$\Longrightarrow$%
      static_live_chan env_a entr exit xc (Let x (Spwn ec) e) |
  Let_Sync:
      static_live_chan env_a entr exit xc e %$\Longrightarrow$%
      entr (top_label e) = exit (NLet x) %$\Longrightarrow$%
      (exit (NLet x) - {x}) %$\cup$% chan_set env_a entr xc xe = entr (NLet x) %$\Longrightarrow$%
      static_live_chan env_a entr exit xc (Let x (Sync xe) e) |
  Let_Fst:
      static_live_chan env_a entr exit xc e %$\Longrightarrow$%
      entr (top_label e) = exit (NLet x) %$\Longrightarrow$%
      (exit (NLet x) - {x}) %$\cup$% chan_set env_a entr xc xa = entr (NLet x) %$\Longrightarrow$%
      static_live_chan env_a entr exit xc (Let x (Fst xa) e) |
  Let_Snd:
      static_live_chan env_a entr exit xc e %$\Longrightarrow$%
      entr (top_label e) = exit (NLet x) %$\Longrightarrow$%
      (exit (NLet x) - {x}) %$\cup$% chan_set env_a entr xc xa = entr (NLet x) %$\Longrightarrow$%
      static_live_chan env_a entr exit xc (Let x (Snd xa) e) |
  Let_Case:
      static_live_chan env_a entr exit xc e %$\Longrightarrow$%
      entr (top_label e) = exit (NLet x) %$\Longrightarrow$%
      static_live_chan env_a entr exit xc el %$\Longrightarrow$%
      static_live_chan env_a entr exit xc er %$\Longrightarrow$%
      (exit (NLet x) - {x}) %$\cup$% chan_set env_a entr xc xs %$\cup$% 
      (entr (top_label el) - {xl}) %$\cup$%
      (entr (top_label er) - {xr}) = entr (NLet x) %$\Longrightarrow$%
      static_live_chan env_a entr exit xc (Let x (Case xs xl el xr er) e) |
  Let_App:
      static_live_chan env_a entr exit xc e %$\Longrightarrow$%
      entr (top_label e) = exit (NLet x) %$\Longrightarrow$%
      (exit (NLet x) - {x}) %$\cup$%
      chan_set env_a entr xc f %$\cup$%
      chan_set env_a entr xc xa = entr (NLet x) %$\Longrightarrow$%
      static_live_chan env_a entr exit xc (Let x (App f xa) e)
...
\begin{lstlisting}[style=codestyle1, escapechar=\%]
  ...
  Next: "
    ...
    %$\not$% Set.is_empty (exit l) %$\Longrightarrow$%
    %$\not$% Set.is_empty (entr l') %$\Longrightarrow$%
    static_live_traversable F entr exit (l, ENxt, l')
  " |
  Spawn: "
    (l, ESpwn, l') %$\in$% F %$\Longrightarrow$%
    %$\not$% Set.is_empty (exit l) %$\Longrightarrow$%
    %$\not$% Set.is_empty (entr l') %$\Longrightarrow$%
    static_live_traversable F entr exit (l, ESpwn, l')
  " |
  Call_Live_Outer: "
    (l, ECall, l') %$\in$% F %$\Longrightarrow$%
    %$\not$% Set.is_empty (exit l) %$\Longrightarrow$%
    static_live_traversable F entr exit (l, ECall, l')
  " |
  Call_Live_Inner: "
    (l, ECall, l') %$\in$% F %$\Longrightarrow$%
    %$\not$% Set.is_empty (entr l') %$\Longrightarrow$%
  ...
  Return: "
    ...
    %$\not$% Set.is_empty (entr l') %$\Longrightarrow$%
    static_live_traversable F entr exit (l, ERtn x, l')
  " |
  Send: "
    ((NLet xSend), ESend xE, (NLet xRecv)) %$\in$% F %$\Longrightarrow$%
    {xE} %$\subseteq$% (entr (NLet xSend)) %$\Longrightarrow$%
    static_live_traversable F entr exit ((NLet xSend), ESend xE, (NLet xRecv))
    "
...
inductive static_live_traceable :: "abstract_env %$\Rightarrow$% transition_set %$\Rightarrow$% label_map %$\Rightarrow$% label_map %$\Rightarrow$% label %$\Rightarrow$% (label %$\Rightarrow$% bool) %$\Rightarrow$% abstract_path %$\Rightarrow$% bool" where
  Empty:
    is_end start %$\Longrightarrow$%
    static_live_traceable env_a F entr exit start is_end [] |
  Edge:
    is_end end %$\Longrightarrow$%
    static_live_traversable F entr exit (start, edge, end) %$\Longrightarrow$%
    static_live_traceable env_a F entr exit start is_end [(start, edge)] |
  Step:
    static_live_traceable env_a F entr exit middle is_end ((middle, edge') # path) %$\Longrightarrow$%
    static_live_traversable F entr exit (start, edge, middle) %$\Longrightarrow$%
    static_live_traceable env_a F entr exit start is_end ((start, edge) # (middle, edge') # path) |
  Pre_Return:
    static_live_traceable env_a F entr exit (NRslt y) is_end ((NRslt y, ERtn x) # post) %$\Longrightarrow$%
    static_traceable  F (NRslt y) pre %$\Longrightarrow$%
    %$\not$% static_balanced (pre @ [(NRslt y, ERtn x)]) %$\Longrightarrow$%
    %$\not$% Set.is_empty (exit (NLet x)) %$\Longrightarrow$%
    path = pre @ (NRslt y, ERtn x) # post %$\Longrightarrow$%
    static_live_traceable env_a F entr exit start is_end path
...
inductive static_one_shot :: abstract_env %$\Rightarrow$% exp %$\Rightarrow$% var %$\Rightarrow$% bool where
  Sync:
    every_two (static_live_traceable env_a F entr exit (NLet xC) (static_send_label env_a e xC)) singular %$\Longrightarrow$%
    static_live_chan env_a entr exit xC e %$\Longrightarrow$%
    static_traversable env_a graph (static_recv_label env_a e) e %$\Longrightarrow$%
    static_one_shot env_a e xC

inductive static_one_to_one :: abstract_env %$\Rightarrow$% exp %$\Rightarrow$% var %$\Rightarrow$% bool where
  Sync:
    every_two (static_live_traceable env_a F entr exit (NLet xC) (static_send_label env_a e xC)) noncompetitive %$\Longrightarrow$%
    every_two (static_live_traceable env_a F entr exit (NLet xC) (static_recv_label env_a e xC)) noncompetitive %$\Longrightarrow$%
    static_live_chan env_a entr exit xC e %$\Longrightarrow$%
    static_traversable env_a graph (static_recv_label env_a e) e %$\Longrightarrow$%
    static_one_to_one env_a e xC
